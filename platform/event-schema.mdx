---
title: Event Schema
description: "Schema reference for TrustGuard and TrustGate metadata events — the fields AlertEngine rules match on, how detection normalization works, and what is not stored."
---

# Event schema

[Alerts](/platform/alerts) and [Use Cases](/platform/use-cases) evaluate rules over **metadata events** stored in the NeuralTrust metadata store. TrustGuard and TrustGate each emit one metadata record per request over **OpenTelemetry (OTLP)**; a collector ingests them into separate **TrustGate** and **TrustGuard** event streams.
...
flowchart LR
  ...
  COL --> STORE[(Metadata store)]
  STORE --> AE[AlertEngine]
  AE --> AL[Alerts UI]
...
|---|---|---|
| Metadata + findings | NeuralTrust metadata store (TrustGate and TrustGuard streams) | Rules, alerts, logs list |
...
| Logical group key | Stored as | Notes |
|---|---|---|
...
| Logical field | Stored as / JSON | Notes |
|---|---|---|
...
| `cost.*` | `cost` JSON | Cost in USD. TrustGate rates: [registry contract pricing](/trustgate/concepts/registries#contract-pricing) when set. |
| `latency.*` | `latency` JSON | Breakdown (provider, gateway, policies, routing, …) |
| `mcp.*` | `mcp` JSON | MCP call metadata |
| `end_user` | `end_user` | The person a client application declared it was serving — one identifier. Caller-asserted, never verified: see [End-user attribution](/trustgate/observability/end-user-attribution). |
| `end_user.*` | `end_user_detail` JSON | The whole declaration: `id`, `email`, `name`, `role`, `source`. |
| `principal.*` | `principal_subject` / `principal_email` / `principal_method` | The credential that actually authenticated. Distinct from `end_user` on purpose. |
...
- Cost sub-fields: `cost.prompt_usd`, `cost.completion_usd`, `cost.total_usd`, `cost.currency`
...
<CardGroup cols={2}>
  ...
  <Card title="TrustGate metrics worker" icon="chart-line" href="/trustgate/observability/metrics">
