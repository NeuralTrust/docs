...
Connect a destination in the **Integrations** view to stream every new alert to your security tooling as an OCSF Detection Finding.

| Provider | Transport |
|---|---|
| Microsoft Sentinel | Azure Monitor Logs (DCR) |
| Datadog | Logs intake API |
| Splunk | HTTP Event Collector (HEC) |
| Elastic | Elasticsearch document API |
| IBM QRadar | LEEF over syslog (TLS) |
| Webhook | Generic HTTP POST (optional HMAC signature) |

A connected integration forwards **all** matching alerts by default. Optional **filters** narrow the stream by minimum severity, source, or an explicit severity allow‑list. Connection secrets are validated when you save and encrypted at rest, and failed deliveries are retried with backoff and surfaced for inspection.

<Note>
This is distinct from [platform SIEM integration](/platform/siem-integration), which forwards **audit logs**. Telemetry Alerts forward **security findings** derived from TrustGuard and TrustGate telemetry.
</Note>
