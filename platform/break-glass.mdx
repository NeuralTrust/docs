...
3. When **Enforce SSO** is on, members on a **verified** email domain use the
   configured IdP. A collaborator who cannot complete the IdP — for example an
   external-domain guest — is granted access through a Break the Glass account (see
   [SSO](/platform/sso#part-5-enable-sso-only-mode-optional)), which still uses
   password.
...
| SSO Enforcement ON | Verified-domain members use IdP; anyone who cannot complete the IdP needs a Break the Glass account | **Password only** |
