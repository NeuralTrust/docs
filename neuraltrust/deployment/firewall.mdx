...
description: Deploy the NeuralTrust Firewall — gateway plus specialized workers for prompt and response safety, on CPU or GPU.
---

The NeuralTrust Firewall provides in-cluster prompt and response safety for TrustGate and the Control Plane. It deploys as a lightweight **gateway** plus a pool of five specialized **workers** (toxicity, toolguard, prompt-jailbreak, prompt-moderation, response-jailbreak). The same chart and image versions support both CPU and GPU inference.

A zero-config install ships the gateway plus five **CPU** workers (`neuraltrust-firewall.firewall.enabled: true`). To trade cost for inference latency, move the workers to a separate **GPU** node pool — see [CPU vs GPU](#cpu-vs-gpu) below.
...
- [Install on Kubernetes](./kubernetes/overview) — base install workflow
- [Secrets management](./secrets) — `trustgate-secrets` and `firewall-secrets` lifecycle
- [OpenShift](./openshift/overview) — SCC and node-pool considerations for GPU workers
