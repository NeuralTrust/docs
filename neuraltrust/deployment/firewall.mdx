...
The firewall subchart is **on by default** (`neuraltrust-firewall.firewall.enabled: true`) — a zero-config install ships a gateway + five CPU workers. Disable it with `neuraltrust-firewall.firewall.enabled: false` if you'd rather call NeuralTrust-hosted firewall endpoints from TrustGate.
