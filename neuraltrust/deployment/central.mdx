...
remote cluster climbs the standard ladder — one cluster across zones first, then
twin clusters, and only for regional loss an active/passive or active/active pair.
...
Most central clusters should sit on **tier 1**: one cluster across three zones
with managed stores. It is not on the request path, so a promotion runbook is
rarely worth its own operational cost.

If a regional requirement forces tier 3, run it as an
[External active/passive pair](/neuraltrust/deployment/external#high-availability)
— same shape, same ClickHouse caveat, same promotion steps. Two things are
...
Either install both bundles in every remote cluster, or issue the four
certificates from your own PKI.

**The platform Secret must be identical in both central clusters.**
`ENROLMENT_SIGNING_SECRET` and `TELEMETRY_JWT_PRIVATE_KEY_PEM` sign the enrolment
and OTLP tokens every remote agent presents. If each cluster generates its own,
tokens minted by one are rejected by the other and every remote agent returns
401 after a promotion. Pre-provision the Secret and apply the same one to both —
see [Secrets](#secrets).

## Next steps

<CardGroup cols={2}>
  <Card title="Hybrid" icon="cloud-upload" href="/neuraltrust/deployment/hybrid">
  ...
  <Card title="Config sync" icon="refresh-cw" href="/neuraltrust/deployment/config-sync">
