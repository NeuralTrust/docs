...
| Min CPU nodes | **≥ 4 (CPU Firewall)**. Subtract one when using GPU Firewall workers. Across 3 AZs for HA. |
...
- **PostgreSQL**: use Azure Database for PostgreSQL Flexible Server with built-in PITR; disable `neuraltrust-control-plane.infrastructure.postgresql.deploy`. This store holds TrustGate admin metadata **and** the raw prompt/response payloads.
- **Redis**: use Azure Cache for Redis, or run the in-cluster Redis.
