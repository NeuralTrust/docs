...
| --- | --- | --- | --- |
| PostgreSQL | **Managed** | Control-plane schema, plus a separate `datacore` database and one per product | Pre-create the databases and roles; Helm does not create users in managed services |
| ClickHouse | **In-cluster** | Telemetry and every dashboard metric | DataCore migrates its own tables at startup |
| Redis | **Managed** | Gateway state | Required at boot |

That split is deliberate. Postgres and Redis are almost always managed services in production, so the usual external values file sets `deploy: false` for both and points at your endpoints. ClickHouse is the opposite: nearly everyone runs the in-cluster subchart, because it is telemetry storage the platform owns end to end. Connecting an external ClickHouse is supported but rare — treat it as the exception, not the starting point.

If you supply your own Postgres Secret rather than letting the chart build one, mind the [`DB_*` key contract](/neuraltrust/deployment/secrets#bringing-your-own-postgres-secret) — the keys differ from the ones the chart writes itself.
