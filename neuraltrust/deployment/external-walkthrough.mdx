...
Only the control-plane `neuraltrust` password still needs to be readable at render time (`global.postgresql.password`). Replacing `postgresql-secrets` wholesale with `global.postgresql.existingSecret` is a different contract (Hybrid / full Secret takeover) and is usually the worse path for External.
