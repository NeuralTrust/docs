...
<Note>
`create-secrets.sh` in the chart repository has drifted behind this list. It prompts for many credentials the chart now generates, and does not prompt for either of the two that are genuinely mandatory. Create the Secrets by hand as shown in [Console setup](/neuraltrust/deployment/console-setup) instead.
</Note>
