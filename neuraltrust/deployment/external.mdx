...
global:
  postgresql:
    ...
    sslMode: "require"
    password: "<pw-control-plane>"   # neuraltrust role — needed at render time for DSNs
  redis:
    deploy: false
...
Runtime role passwords can stay out of the values file. Point each service at a
Secret you created — see
[External per-service datastore credentials](/neuraltrust/deployment/secrets#external-per-service-datastore-credentials).
`global.postgresql.password` is the one credential that still belongs in values
(or `--set`): the chart bakes it into the console connection strings at render
time.
