...
NeuralTrust images are private. When your account is provisioned, NeuralTrust
sends you a **registry key** — a JSON service-account key — plus the registry
server to use it against. Keep it with your other production credentials; it is
the only thing standing between your cluster and the images.
...
Create a `docker-registry` Secret from the key. The chart repository ships
...
```bash
helm template neuraltrust-platform <chart> -f <your-values.yaml> \
  | grep -o 'image: .*' | sort -u
```
