...
| `clickstack-collector.neuraltrust.ai` | resolve via DNS | Product metadata OTLP export |

Product metadata OTLP does not leave the pods directly: a co-located egress
collector forwards to `clickstack-collector.neuraltrust.ai`, so that is the
host to allow if your policy lists every external destination.
