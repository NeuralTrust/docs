...
| **External** | Same global blocks — per-service hosts inherit since chart 2.6.0 | Control-plane password in `global.postgresql.password`; runtime roles via per-service [`existingSecret` hooks](/neuraltrust/deployment/secrets#external-per-service-datastore-credentials) |
