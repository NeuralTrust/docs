...
across regions, they share **one writable PostgreSQL primary** with a
cross-region read replica, and each region gets **its own Redis** — see
[High availability](/neuraltrust/deployment/high-availability).
...
### TrustGate data-plane environment

Optional variables you may set on the TrustGate proxy and MCP pods, beyond what the
chart and the wizard generate.

| Variable | Guidance | Effect |
| --- | --- | --- |
| `MCP_OAUTH_CLIENT_NAME` | One name per environment | The `client_name` TrustGate registers with upstream MCP authorization servers through dynamic client registration — what users see on the upstream consent screen. Defaults to `TrustGate MCP Gateway`. Changing it re-registers the cached clients, and grants held by the previous registration can no longer be refreshed. |
| `PLAYGROUND_TOKEN_PUBLIC_KEYS` | Usually empty in Hybrid | RS256 public keys that verify playground and diagnostics tokens minted by the control plane; a bare PEM or a JSON array of `{kid, pem}`. Falls back to `ADMIN_M2M_PUBLIC_KEYS` when unset. Config-sync data planes also receive these keys in the snapshot. |
| `PROXY_SERVE_HYBRID_GATEWAYS` | Leave the default | Whether this proxy serves gateways entitled as `data_plane=hybrid`. Defaults to the value of `CONFIG_SYNC_DATA_PLANE_ENABLED`, so a config-sync data plane serves them and a hosted proxy answers **421** for them instead. Set `true` only on a customer-run data plane that is not DB-less. |
