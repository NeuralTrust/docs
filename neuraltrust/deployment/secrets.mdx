...
- Alongside it, `CONFIG_SYNC_LKG_KEY` encrypts the last-known-good configuration snapshot on disk. It is a base64-encoded 32-byte key you generate, not a token the console issues, and it must differ per product.
...
**Configuration sync** — Secrets holding `CONFIG_SYNC_TOKEN` and `CONFIG_SYNC_LKG_KEY`:
