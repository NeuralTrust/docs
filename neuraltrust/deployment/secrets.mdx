...
1. Regenerate the token in the console (or issue a new chart secret value).
2. Update the Kubernetes Secret (or secret manager) with the new value.
3. Roll the affected Deployments so pods pick up the new secret.

Old tokens stop working after regeneration — update Secrets before or immediately after rotate.

<Warning>
With GitOps, keep `autoGenerateSecrets: false` and `preserveExistingSecrets: true`, and manage Secret manifests yourself. Chart auto-generation can fight GitOps and rotate values unexpectedly.
</Warning>

See the chart's [`SECRETS.md`](https://github.com/NeuralTrust/neuraltrust-platform/blob/main/SECRETS.md) in `neuraltrust-platform` for the full secret-rotation and GitOps guidance.
...
Hybrid exports privacy-safe metadata via OTLP; raw payloads stay local — see [Deployment models](/neuraltrust/deployment/deployment-models).
...
Full key lists live in the chart repository's [`SECRETS.md`](https://github.com/NeuralTrust/neuraltrust-platform/blob/main/SECRETS.md).
