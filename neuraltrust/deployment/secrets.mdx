...
it with `--set` from your secret store at install time.
...
DataAgent reads as its `DATABASE_URL`. `POSTGRES_LOGIN` is the authentication
switch — `default` for password auth, `aws` for IAM.
...
global:
  postgresql:
    ...
    sslMode: "require"
    password: "<pw-control-plane>"   # still required at render time for DSNs
  redis:
    deploy: false
...
An inline `password` next to a hook is rejected at render. The hooks are ignored
under `iamAuth: true` and in Hybrid (which already has
