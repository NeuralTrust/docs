...
For the on/off toggles that change which secrets are needed (in-cluster vs external Postgres and Redis; image registry; storage class; secret modes), see the [Feature flags reference](./feature-flags).
