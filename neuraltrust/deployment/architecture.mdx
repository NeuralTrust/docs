...
reachable. For the topology *decision* — SaaS, Hybrid, External, or a Central
...
## Central control plane (`saas`)

A superset of External, spread across clusters. The central cluster runs
everything External runs, plus three components that let data planes in **other**
clusters enrol into it. Each of those data planes is an ordinary Hybrid install
pointed at your domain rather than at NeuralTrust.

```mermaid
flowchart TB
  subgraph CEN["Central cluster — deploymentMode: saas"]
    direction TB
    CP["Console · control-plane-api · AlertEngine<br/>TrustGate + TrustGuard config-sync"]
    DC["DataCore<br/>mints enrolment + OTLP tokens<br/>serves JWKS"]
    DB["DataBridge<br/>northbound 50051 in-cluster<br/>southbound 443 public"]
    GW["ClickStack ingest gateway<br/>verifies OTLP tokens"]
    COL["ClickStack collector"] --> CH[("ClickHouse")]
    DC --> DB
    GW --> COL
    DB -.->|introspect| DC
    GW -.->|JWKS| DC
  end
  subgraph REM["Data-plane cluster N — deploymentMode: hybrid"]
    direction TB
    PR["TrustGate / TrustGuard"]
    DA["DataAgent<br/>unique instance ID"]
    EG["egress collector"]
  end
  DA -->|"gRPC 443 + enrolment token"| DB
  EG -->|"OTLP 443 + bearer token"| GW
  PR -.->|"config-sync gRPC 443"| CP
```

| Addition | Deployed when | Purpose |
| --- | --- | --- |
| DataBridge | `deploymentMode: saas` | Holds a bidirectional gRPC stream per remote DataAgent; DataCore queries across them |
| ClickStack ingest gateway | `deploymentMode: saas` | Public OTLP edge that verifies tokens and stamps the tenant from the verified claim |
| Published config-sync Services | `deploymentMode: saas` | Lets remote products reach the central control planes |

DataCore also switches to a hybrid residency backend, so entitled reads for a
remote data plane go out through DataBridge instead of to the local ClickHouse.
Raw payloads never move: they stay in the cluster that produced them.

See [Central control plane](/neuraltrust/deployment/central) for endpoints, TLS,
and authentication.
...
A Central control plane inverts the ingress side: each data-plane cluster needs
the same outbound set as Hybrid but against **your** domain, and the central
cluster has to **accept** those connections on four published endpoints.

The authoritative hostname and IP list, which you should use rather than copying
values from this page, is the
