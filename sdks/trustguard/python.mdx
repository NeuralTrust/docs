...
| Requirement | Notes |
|---|---|
| A **Python SDK** [collector](/trustguard/concepts/collectors) and API key | Go to **Agent Runtime → Collectors → Catalog → Application → Python**. Create the `tgk_…` key on the **Auth** tab, store it when shown, and load it from your environment or secret store. |
...
The package is [`trustguard-sdk` on PyPI](https://pypi.org/project/trustguard-sdk/); the
[repository](https://github.com/NeuralTrust/trustguard-sdk/tree/main/python)
holds the source and examples.
</Step>

<Step title="Guard the input" titleSize="h2" id="2-guard-the-input">
...
<Step title="Guard the output" titleSize="h2" id="3-guard-the-output">
...
| --- | --- | --- |
| Before the model call | `protocol: "llm"`, `direction: "input"` | Jailbreaks ([Prompt Guard](/trustguard/detectors/content-security#prompt-guard)); secrets and PII in the prompt ([DLP](/trustguard/detectors/data-loss-prevention)); [injection riding in a retrieved document](/trustguard/detectors/agent-mcp-security) |
...
Other languages and runtimes: [Node.js SDK](/sdks/trustguard/node)
...
- [Node.js SDK](/sdks/trustguard/node) · [REST](/integrations/rest): use the same contract in other runtimes
