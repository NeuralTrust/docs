...
For an agent built on the Vercel AI SDK, use the
[Vercel AI SDK integration](/integrations/vercel-ai-sdk), which ships in this
package.
...
| Requirement | Notes |
|---|---|
| An **Application** [collector](/trustguard/concepts/collectors) and API key | Go to **Agent Runtime → Collectors → Catalog → Application**. The same collector and key work with the SDK and [Node.js middleware](/integrations/node-middleware). Create the `tgk_…` key on the **Auth** tab and store it when shown. |
...
The package is [`@neuraltrust/trustguard-sdk` on npm](https://www.npmjs.com/package/@neuraltrust/trustguard-sdk); the
[repository](https://github.com/NeuralTrust/trustguard-sdk/tree/main/node)
holds the source and examples.
</Step>

<Step title="Create the client" titleSize="h2" id="2-create-the-client">
```ts
...
<Step title="Guard both directions" titleSize="h2" id="3-guard-both-directions">
...
| --- | --- | --- | --- |
| Before the model call | `payload: { input }`, `direction: "input"`, `protocol: "llm"` | Jailbreaks ([Prompt Guard](/trustguard/detectors/content-security#prompt-guard)); secrets and PII in the prompt ([DLP](/trustguard/detectors/data-loss-prevention)) | **Block** or **Redact**, by your code |
...
| Option | Notes |
|---|---|
| `baseUrl` | `{TRUSTGUARD_BASE_URL}` for your workspace, from the console. |
...
- [Python SDK](/sdks/trustguard/python): use the same API from Python
