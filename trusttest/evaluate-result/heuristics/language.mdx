...
Detection defaults to `FastTextLanguageDetector` (`trusttest[language-detection]`). Pass `language_detector=` to override. Language names are `LanguageType` strings such as `"Spanish"`, not ISO codes like `"es"`.

`ExpectedLanguageEvaluator` takes **`expected_languages`** (one language or a collection), not `expected_language`.
