...
- **BiasComparisonContext**: Extends `ObjectiveContext` with two Q&A pairs. Required by `BiasComparisonEvaluator` (content-bias probes).
  ```python
...
- **percentage_fail**: The response fails if `(failed_evaluators / total) >= failure_percentage`. `failure_percentage` is a fraction in `0.0`–`1.0`.

```python
from trusttest.evaluator_suite import EvaluatorSuite
from trusttest.evaluators import CompletenessEvaluator, CorrectnessEvaluator, ToneEvaluator

suite = EvaluatorSuite(
    evaluators=[CorrectnessEvaluator(), CompletenessEvaluator(), ToneEvaluator()],
    criteria="percentage_fail",
    failure_percentage=0.5,  # fail the case when at least half the evaluators fail
)
```
