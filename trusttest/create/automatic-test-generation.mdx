...
<Note>
Vector knowledge bases need `set_config` for `embeddings` and `topic_summarizer` (and an LLM for question generation). Topic clustering uses UMAP + HDBSCAN on **any** embedding-backed connector — it is not Azure-only. Install `trusttest[language-detection]` if you want FastText language detection on ingested documents.
</Note>
...
`RagContextBuilder` writes gold `expected_response` values from retrieved documents so you can judge with `CorrectnessEvaluator` / `CompletenessEvaluator` instead of only relevance.
...
kb = InMemoryKnowledgeBase(
    documents=[
        Document(id="1", content="TrustTest evaluates AI models.", topic="product"),
    ]
)
questions = [QuestionTopic(question="What does TrustTest evaluate?", topic="product")]

builder = RagContextBuilder(
    questions=questions,
    knowledge_base=kb,
)
dataset = builder.build()  # Dataset[ExpectedResponseContext]; or await builder.async_build()
```

Constructor: `questions: list[QuestionTopic]`, `knowledge_base`, optional `llm_client`, optional `system_prompt`.
