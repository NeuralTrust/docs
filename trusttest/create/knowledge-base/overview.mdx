...
description: Connectors used to generate RAG and functional tests from your documents.
---

<p className="nt-lead">
  A knowledge base is a collection of documents used to generate test cases —
  usually the vector store behind retrieval-augmented generation. Documents are
  grouped by topics. If you omit <code>seed_topics</code>, the connector clusters
  embeddings and names topics with an LLM.
</p>

<CardGroup cols={2}>
  <Card title="In memory" icon="cpu" href="/trusttest/create/knowledge-base/connectors/in-memory">
    Local documents. No extra service.
  </Card>
  <Card title="Azure AI Search" icon="cloud" href="/trusttest/create/knowledge-base/connectors/azure">
    `AzureKnowledgeBase` over an existing search index.
  </Card>
  <Card title="Neo4j" icon="share-2" href="/trusttest/create/knowledge-base/connectors/neo4j">
    Graph-shaped document relationships.
  </Card>
  <Card title="Postgres" icon="database" href="/trusttest/create/knowledge-base/connectors/postgres">
    `pgvector` on tables you already run.
  </Card>
  <Card title="Upstash Vector" icon="unplug" href="/trusttest/create/knowledge-base/connectors/upstash">
    Serverless similarity search.
  </Card>
  <Card title="Automatic generation" icon="wand" href="/trusttest/create/automatic-test-generation">
    Turn a knowledge base into a functional or RAG probe.
  </Card>
</CardGroup>
...
This process is triggered when no predefined (`seed_topics`) list is provided. Configure `embeddings` and `topic_summarizer` with `set_config` (or pass `embeddings_model` / `llm_client`).
