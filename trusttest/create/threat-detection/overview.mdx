...
description: Catalog of TrustTest attack probes — prompt injection, leaks, unsafe outputs, and agent misuse.
---

<p className="nt-lead">
  Each probe generates test cases, sends them to the target, and collects
  responses for evaluation. Start with prompt injection, then add the categories
  that match the system you are shipping.
</p>

<CardGroup cols={2}>
  <Card title="Prompt injections" icon="syringe" href="/trusttest/create/threat-detection/prompt-injections/overview">
    Single-turn, multi-turn, and dataset attacks.
  </Card>
  <Card title="Content bias" icon="scale" href="/trusttest/create/content-bias">
    Cognitive and stereotypical bias probes.
  </Card>
  <Card title="Sensitive data leak" icon="key-round" href="/trusttest/create/sensitive-data-leak">
    Extract confidential data, tools, or reasoning.
  </Card>
  <Card title="System prompt disclosure" icon="scroll-text" href="/trusttest/create/system-prompt-disclosure">
    Attempts to extract internal instructions.
  </Card>
  <Card title="Input leakage" icon="messages-square" href="/trusttest/create/input-leakage">
    Whether prior conversation content leaks across boundaries.
  </Card>
  <Card title="Unsafe outputs" icon="octagon-alert" href="/trusttest/create/unsafe-outputs">
    Harmful content, illegal activity, and malicious artifacts.
  </Card>
  <Card title="Off-topic" icon="compass" href="/trusttest/create/off-topic">
    Whether the model stays inside its intended scope.
  </Card>
  <Card title="Agentic behavior" icon="bot" href="/trusttest/create/agentic-behavior">
    Stop-command override, tool misuse, and long-horizon drift.
  </Card>
</CardGroup>
