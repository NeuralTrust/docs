...
description: Single-turn, multi-turn, and dataset prompt-injection probes in TrustTest.
---

<p className="nt-lead">
  Prompt injection tries to make the model ignore its instructions. TrustTest
  groups probes by technique so you can start with a single message, then add
  multi-turn and dataset attacks.
</p>

<CardGroup cols={3}>
  <Card title="Single turn" href="/trusttest/create/threat-detection/prompt-injections/single-turn/overview">
    Jailbreaks, encoding, structural, language, multimodal, MCP, and RAG probes.
  </Card>
  <Card title="Multi-turn" href="/trusttest/create/threat-detection/prompt-injections/multi-turn/overview">
    Crescendo, Echo Chamber, payload splitting, and conditioning.
  </Card>
  <Card title="From dataset" href="/trusttest/create/threat-detection/prompt-injections/from-dataset">
