...
An `EvaluatorSuite` is a collection of `Evaluator` objects that work together to comprehensively evaluate an LLM's response. It combines multiple evaluators' results to determine if a test case passes or fails based on defined criteria (`all_fail`, `any_fail`, `one_fail`, or `percentage_fail` with `failure_percentage` in `0.0`–`1.0`).
