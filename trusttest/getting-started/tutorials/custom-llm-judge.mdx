...
Create a custom LLM judge with **`CustomEvaluatorExpected`** when each test case has a gold answer (`ExpectedResponseContext`). Use **`CustomEvaluatorObjective`** when you only have pass/fail descriptions (`ObjectiveContext`). There is no `CustomEvaluator` class.
