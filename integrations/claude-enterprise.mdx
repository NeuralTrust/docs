...
| The **Claude Enterprise** collector type | **Runtime → Collectors → Catalog → IDE & coding agents → Claude Enterprise**. Create the `tgk_…` key on **Auth**, then assign a policy with the required Input rules on **Policies**. The key is shown once. |
