...
| **[TrustGate](/trustgate/overview)** | Exposes the MCP registries and tools assigned to an application. MCP (Model Context Protocol) connects Claude to systems such as trackers, databases, and internal APIs. | Tool availability · application authentication · per-tool rate limits |
...
| Centrally manage the MCP tools available to Claude | [Organization connector](#organization-connectors) using a TrustGate [MCP application](/trustgate/mcp/overview) | Anthropic's cloud calls the MCP endpoint |
...
`Authorization` header. Organization connectors authenticate MCP applications with
...
organization connector, each member completes an OAuth login against the identity
provider the application trusts.
...
merged toolkit assigned to the MCP application. Copy the URL from the application's
**Connect** tab:
...
API-key applications work through the [Claude Code
...
**Connector ownership.** An Anthropic organization owner adds the MCP application's
...
Which IdP backs the login is configured on the application. See
...
| Connector connects, no tools | Application has no bound registries, tool restrictions exclude everything, or the upstream connect link was never authorized |
| `401` / repeated login on MCP | Check the application's plane URL and authentication configuration. For a CLI connection, also check for a revoked `ag_…` key or missing `X-AG-Gateway-Slug` on Hybrid |
