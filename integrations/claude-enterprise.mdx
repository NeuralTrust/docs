...
| Requirement | Notes |
| --- | --- |
| Egress to `{TRUSTGUARD_URL}` | The console shows the URL for your workspace. Anthropic's servers must be able to reach it. |
| The **Claude Enterprise** collector type | **Agent Runtime → Collectors → Catalog → IDE & coding agents → Claude Enterprise**. Create the `tgk_…` key on **Auth**, then assign a policy with the required Input rules on **Policies**. The key is shown once. |
