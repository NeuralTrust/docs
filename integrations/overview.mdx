...
| [Windsurf](/integrations/windsurf) | — | TrustGate MCP server for Cascade (`serverUrl` in `mcp_config.json`) |
...
| [Microsoft Copilot Studio](/integrations/copilot-studio) | Threat detection evaluates every tool the agent is about to run, authenticated with an Entra federated credential | An MCP tool pointed at a TrustGate application |
...
Input and output enforcement varies by gateway. Non-streaming completions can be
blocked or rewritten before the client sees them when the adapter supports it.
On a stream, [LiteLLM](/integrations/litellm) scans the growing completion and
can stop later chunks. [TrustGate](/integrations/trustgate) inspects after
delivery, so streamed output there is monitor-only. Direct provider calls
bypass the gateway.
...
| [LiteLLM](/integrations/litellm) | Custom `apply_guardrail` file; scans during streaming |
...
| [Haystack](/integrations/haystack) | `neuraltrust-haystack`, pipeline components for text and text-only chat input and completed replies |
...
These integrations trace the calls your application sends through TrustGate in
the observability tool you already use. Tracing runs in your application, next
to TrustGate's own record of each call in **Activity**.

| Tool | Integration method |
| --- | --- |
| [Langfuse](/integrations/langfuse) | Langfuse's OpenAI integration wraps a client configured by the TrustGate SDK from the application key |
