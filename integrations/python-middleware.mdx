...
| Requirement | Notes |
|---|---|
| An **Application** [collector](/trustguard/concepts/collectors) and API key | Go to **Agent Runtime → Collectors → Catalog → Application**. The middleware uses the same package and key as the [Python SDK](/integrations/python-sdk). Create the key on the **Auth** tab, store it when shown, and load it from your environment or secret store. |
