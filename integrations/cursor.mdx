...
| The **Cursor** collector type *(firewall path)* | **Agent Runtime → Collectors → Catalog → IDE & coding agents → Cursor**. Create the `tgk_…` key on its **Auth** tab, where it is shown once, and assign the policy on the **Policies** tab. |
