...
| **[TrustGate](/trustgate/overview)** | Exposes the MCP registries and tools assigned to an application. MCP (Model Context Protocol) connects Cursor to systems such as trackers, databases, and internal APIs. | Tool availability · application authentication · per-tool rate limits |
...
authenticates as an application with OAuth or an `ag_…` API key. A `tgk_…` key does
...
| The **Cursor** collector type *(firewall path)* | **Agent Runtime → Collectors → Catalog → AI assistants & coding agents → Cursor**. Create the `tgk_…` key on its **Auth** tab, where it is shown once, and assign the policy on the **Policies** tab. |
...
| An [MCP application](/trustgate/mcp/overview) *(MCP path)* | Bind the required registries, then copy the endpoint or Cursor snippet from **Connect**. |
| Application credentials *(MCP path)* | Use OAuth, which requires no pasted credential, or an `ag_…` API key. |
...
Cursor treats TrustGate as a **remote MCP server**: one `url` per MCP application,
and the agent sees the merged toolkit that application is routed to. Copy the
endpoint or Cursor snippet from the application's **Connect** tab:
...
| `TRUSTGATE_MCP_API_KEY` | API-key applications only | `X-AG-API-Key`. Leave empty for OAuth |
| `TRUSTGATE_GATEWAY_SLUG` | Hybrid only | `X-AG-Gateway-Slug` |

OAuth applications need only the URL. Cursor starts the login flow on first tool
...
**OAuth applications (recommended).** Omit `headers` and let Cursor run the
handshake:
...
```json
{
  "mcpServers": {
    "TrustGate": {
      "url": "https://<mcp-host>/<application-slug>/mcp"
    }
  }
}
```
...
**API-key applications.** Send the application key as a header:
...
For OAuth applications on Hybrid, send `X-AG-Gateway-Slug` alone.
...
For a team rollout, register the application in the Cursor dashboard instead of
requiring each developer to edit `mcp.json`:

| Location | Purpose |
|---|---|
| **Dashboard → Integrations & MCP** | Register the application as a Team MCP server (also reaches Cloud Agents). **Add to Team Marketplace** makes it installable from **Customize** in the Agent Window, IDE, and CLI |
...
Keep the tool scope on the application or role. The Cursor-side tool allowlist
...
2. Ask the agent to list or use a tool from a toolkit bound to that application.
...
| --- | --- | --- | --- |
| `beforeSubmitPrompt` | `protocol: llm`, `direction: input` | Jailbreaks ([Prompt Guard](/trustguard/detectors/content-security#prompt-guard)); secrets and PII pasted into the agent ([DLP](/trustguard/detectors/data-loss-prevention)) | **Block** |
...
| **Global** | `~/.cursor/mcp.json` | An application every project should reach |
...
needed here. The application configuration determines which IdP handles the login.
See [Auth](/trustgate/concepts/auth).

Use OAuth for applications whose users sign in individually. Every caller an application
admits sees the same registries and toolkit; per-person self-service lives in the
[MCP Store](/trustgate/mcp/store) and is governed from [Access](/trustgate/mcp/access).
...
TrustGate defines the application's tool set. Cursor applies additional settings on
the local machine:

| Decision | Where it lives | Notes |
|---|---|---|
| Which tools the application exposes | **NeuralTrust console**, on the application's **General** tab | Add MCP servers. **Restrict tools** narrows a registry to named tools; leave it empty to grant the whole server |
...
| Hiding a tool from this editor | **Cursor**, through the server toggle in **Customize** | Local only; the application still grants it |

Cursor Run Mode and server visibility apply only to the local client. Configure
restrictions that must apply to every client on the application or role. To limit
MCP tool calls, attach the [Per-Tool Rate
Limiter](/trustgate/policies/per-tool-rate-limiter) policy. The Tool Injection policy
...
| Server connects, no tools | Application has no bound registries, tool restrictions exclude everything, or the upstream connect link was never authorized |
...
| Duplicate tools | The same application is registered through both plugin variables and `mcp.json` |
...
- [How it works](/trustguard/how-it-works): comparison of Cursor with other collectors
