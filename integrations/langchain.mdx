...
| --- | --- | --- |
| **[TrustGuard](/trustguard/overview)** | An `AgentMiddleware` in `create_agent` checks the conversation, model response, tool results, and pending tool calls against the assigned [policy](/trustguard/concepts/policies). The agent acts as a [collector](/trustguard/concepts/collectors). | Monitor · Block · Transform |
...
Policy can also apply across turns rather than to one message at a time, which
requires passing a conversation identifier on every invocation — see
[Group the turns of a conversation](#4-group-the-turns-of-a-conversation).

## Before you start

| Requirement | Notes |
|---|---|
| An **Application** collector and its API key | The middleware is a Python client, so create **Agent Runtime → Collectors → Catalog → Application → Python**. Create the `tgk_…` key on its **Auth** tab, where it is shown once, and assign a policy on the **Policies** tab. There is no LangChain collector type — the key is what identifies the collector. |
| A policy bound to that collector | With Input **and** Output phase rules if you want both directions evaluated. |
| Egress from your app to `{TRUSTGUARD_BASE_URL}` | The console shows the [base URL](/trustguard/api/evaluate#base-url) for your workspace. |
...
<Step title="Install" titleSize="h2" id="1-install">
```bash
...
<Step title="Add the middleware" titleSize="h2" id="2-add-the-middleware">
```python
...
This evaluates one turn in isolation. For a chat application, pass a
conversation identifier on every invocation so the turns are evaluated as one
conversation — see
[Group the turns of a conversation](#4-group-the-turns-of-a-conversation).
...
<Step title="Choose what gets evaluated" titleSize="h2" id="3-choose-what-gets-evaluated">
...
<Step title="Group the turns of a conversation" titleSize="h2" id="4-group-the-turns-of-a-conversation">
...
independently, which is what the middleware does until you give it the
conversation.

Two inputs give TrustGuard the conversation, and they are complementary rather
than alternatives:

| What the middleware sends | What TrustGuard can then do |
|---|---|
| A stable `session_id` on **every** evaluation of one conversation | Group the turns server-side. Findings for one conversation appear together in **Activity**, `session.id` becomes available as a [gate](/trustguard/concepts/policies#gates) condition, and the [stateful detectors](/trustguard/concepts/collectors#attribution) get the grouping they rely on. |
...
so an agent that never passes a `thread_id` produces one session per evaluation.
Nothing errors and no finding goes missing, so the gap is only visible as
single-turn conversations in **Activity**.

### Pass a thread_id on every invocation

The middleware takes `session_id` from the LangGraph `thread_id` in the run
config. Set it per invocation, so one agent and one middleware instance can serve
many conversations:
...
agent = create_agent(
    model="gpt-4o-mini",
    tools=tools,
    middleware=[TrustGuardMiddleware(check_input=True, check_output=True)],
    checkpointer=InMemorySaver(),
)

config = {"configurable": {"thread_id": str(conversation_id)}}
agent.invoke({"messages": [("human", "first turn")]}, config=config)
agent.invoke({"messages": [("human", "second turn")]}, config=config)
```

Both turns evaluate under the same `session_id`, on every enabled stage —
input, output, tool result, and tool call.

The `thread_id` must be a non-blank **string**. LangGraph accepts a `uuid.UUID`
object or an `int` and checkpoints correctly, but the middleware only forwards a
string, so a non-string or empty `thread_id` drops `session_id` without an error.
Call `str()` on whatever your application uses as a conversation identifier.

<Warning>
`session_id` and `TRUSTGUARD_SESSION_ID` are resolved once, when the middleware is
constructed, and they override `thread_id` on every request that instance makes.
On a multi-user server that collapses every conversation into one session. Use them
only for a single-conversation script, and use `thread_id` everywhere else.
</Warning>

### Give one call the history

With a checkpointer and a stable `thread_id`, `before_model` receives the whole
accumulated thread, and the middleware forwards all of it. The input evaluation
payload therefore already contains the earlier turns. Without a checkpointer, the
application passes the transcript it wants evaluated in the `invoke` payload
itself.

The message list is sent untrimmed. It grows by two messages per turn, more when
tools run, and the `payload_tools` schemas are re-sent on every pass through the
agent loop. There is no windowing setting, so raise `timeout` from its `5.0`
second default for long threads and measure against a representative one.

| Stage | What its payload contains |
|---|---|
| Input (`before_model`) | Every message in state, plus the `payload_tools` schemas |
| Output (`after_model`) | The last AI message only |
| Tool result | The tool messages from the previous step only |
| Tool call (`wrap_tool_call`) | The pending call only |

Only the input stage carries history. The other three depend entirely on the
`session_id` grouping for conversational context.

<Warning>
TrustGuard evaluates the messages that are in state when its `before_model` hook
runs. Any middleware that rewrites `state["messages"]` first —
`SummarizationMiddleware`, for example — replaces the turns it condensed, and a
checkpointer persists that rewrite, so those turns are not in the payload. If a
policy needs the original turns, capture one payload and confirm what it
contains. A `SystemMessage` set through `create_agent(system_prompt=…)` is never
in state and never reaches the payload; one placed in `messages` is sent in full
on every input evaluation.
</Warning>
</Step>

<Step title="Verify" titleSize="h2" id="5-verify">
```python
from langchain.agents import create_agent
from langchain_neuraltrust import TrustGuardMiddleware
...
| --- | --- | --- | --- |
| Input | `before_model` | Jailbreaks ([Prompt Guard](/trustguard/detectors/content-security#prompt-guard)); secrets and PII in the prompt ([DLP](/trustguard/detectors/data-loss-prevention)); [indirect prompt injection](/trustguard/detectors/agent-mcp-security) in tool messages already in the thread, and tool poisoning in the `payload_tools` schemas | **Block** or **Transform** |
| Output | `after_model` | Sensitive data in the assembled `AIMessage` | **Block** or **Transform** |
| Tool result | `before_model` | Indirect prompt injection in the previous step's tool output | **Block** or **Transform** |
| Tool call | `wrap_tool_call` | A destructive or out-of-policy call, or sensitive values in its arguments, **before** the tool runs | **Block** or **Transform** (masking the arguments) |
...
`session_id` and `TRUSTGUARD_SESSION_ID` are read once at construction and pin
that value to every request the instance makes. Leave both unset and pass the
conversation identifier per invocation as the LangGraph `thread_id` instead. See
[Group the turns of a conversation](#4-group-the-turns-of-a-conversation).

The middleware never sends `consumer_id`, so a collector attached in **Consumer
ID** routing mode has nothing to match. Attach it in **Default** routing mode.
...
- `session_id`: the LangGraph `thread_id` unless you set it, and omitted when
  neither is a non-blank string. Sent on every stage. See
  [Group the turns of a conversation](#4-group-the-turns-of-a-conversation)
...
The wire `direction` has two values, not four. Only the output stage sends
`direction: "output"`; the input, tool-result, and tool-call stages all send
`direction: "input"`, so all three are evaluated by the policy's Input phase.
...
| Every turn is its own conversation in **Activity** | No `thread_id` was passed in the run config, so `/v1/evaluate` synthesizes a session per evaluation |
| A `thread_id` is passed but the turns still do not group | It is not a non-blank string. A `uuid.UUID` object or an `int` checkpoints correctly but is dropped by the middleware. Wrap it in `str()` |
| Every conversation shares one session | `session_id` or `TRUSTGUARD_SESSION_ID` is set, and it overrides `thread_id` on every request that instance makes |
| An input evaluation is missing earlier turns | No checkpointer, so state holds only what `invoke` passed; or middleware rewrote `state["messages"]` before TrustGuard's `before_model` hook ran |
| Input payloads grow until requests time out | The message list is sent untrimmed and `payload_tools` is re-sent on every pass. Raise `timeout`, or shorten the thread |
...
- [Python SDK](/sdks/trustguard/python): protect calls outside an agent loop
