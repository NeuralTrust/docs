...
| Requirement | Notes |
|---|---|
| A CloudFront collector, API key, and assigned policy | Go to **Agent Runtime → Collectors → Catalog → WAF → AWS CloudFront**. Create the collector, create a key on its **Auth** tab, and assign an Input-phase policy on the **Policies** tab. The key is shown once and identifies the collector. |
| Egress from Lambda@Edge to `{TRUSTGUARD_BASE_URL}` | The console shows the [base URL](/trustguard/api/evaluate#base-url) for your workspace. |
...
from the edge to `{TRUSTGUARD_BASE_URL}`.
...
| Endpoint | `{TRUSTGUARD_BASE_URL}/v1/evaluate` in the handler | The full endpoint, not the host. |
...
| Symptom | Cause |
|---|---|
| No events in **Activity** | No policy is assigned to the collector, Lambda@Edge cannot reach `{TRUSTGUARD_BASE_URL}`, or the function is not associated with the behavior serving those paths |
