...
| **[TrustGate](/trustgate/overview)** | Exposes the MCP registries and tools assigned to an application. MCP (Model Context Protocol) connects Codex to systems such as trackers, databases, and internal APIs. | Tool availability · application authentication · per-tool rate limits |
...
| The **Codex** collector type *(hooks path)* | **Agent Runtime → Collectors → Catalog → AI assistants & coding agents → Codex**. Create the `tgk_…` key on its **Auth** tab, where it is shown once, and assign the policy on the **Policies** tab. |
...
application, and the agent sees the merged toolkit that application is routed to. Copy
the endpoint from the application's **Connect** tab:
...
### OAuth applications (recommended)
...
Combine it with `env_http_headers` for the API key when the application is not
OAuth.
...
1. Run `codex mcp list`, or `/mcp` in the CLI TUI, and confirm **TrustGate** is
   active.
2. Ask the agent to use a tool from a toolkit bound to that application.
...
| --- | --- | --- | --- |
| `UserPromptSubmit` | `protocol: llm`, `direction: input` | Jailbreaks ([Prompt Guard](/trustguard/detectors/content-security#prompt-guard)); secrets and PII pasted into the agent ([DLP](/trustguard/detectors/data-loss-prevention)) | **Block** with `decision: "block"` |
...
|---|---|---|
| **User** | `~/.codex/config.toml` | An application every session should reach |
| **Project** | `.codex/config.toml` | An application tied to one codebase; loaded only for **trusted** projects |
...
and rejects anything else with `invalid_redirect_uri`. The application configuration
...
TrustGate defines the application's tool set. Codex applies additional settings on
the local machine:
...
client. Configure restrictions that must apply to every client on the application
...
| Server initializes, no tools | Application has no bound registries, tool restrictions exclude everything, or the upstream connect link was never authorized |
...
- [How it works](/trustguard/how-it-works): comparison of Codex with other collectors
