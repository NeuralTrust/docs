...
| **[TrustGate](/trustgate/overview)** | Exposes the MCP registries and tools assigned to a consumer. MCP (Model Context Protocol) connects Codex to systems such as trackers, databases, and internal APIs. | Tool availability · consumer authentication · per-tool rate limits |
...
Use OAuth for consumers whose users sign in individually. Every caller a consumer
...
|---|---|---|
| Which tools the consumer exposes | **NeuralTrust console**, under consumer **Routing** | Bind MCP registries. **Restrict tools** narrows a registry to named tools; leave it empty to grant the whole server |
