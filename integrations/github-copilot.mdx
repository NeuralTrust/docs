...
| A collector and its API key *(hooks path)* | **There is no GitHub Copilot collector type in the catalog yet.** Ask NeuralTrust to enable one for your workspace; the hooks read a `tgk_…` key from `copilot.json` and any collector key resolves to its policy. Create the key on the collector's **Auth** tab, where it is shown once, and assign a policy on **Policies**. |
