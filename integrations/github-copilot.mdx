...
| The **GitHub Copilot** collector type *(hooks path)* | **Agent Runtime → Collectors → Catalog → IDE & coding agents → GitHub Copilot**. Create the `tgk_…` key on its **Auth** tab, where it is shown once, and assign the default policy on the **Policies** tab. |
