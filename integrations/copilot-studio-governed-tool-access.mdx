...
| **Rate limits** | [Per-tool limiter](/trustgate/policies/per-tool-rate-limiter) on the consumer | Same |
...
Every caller a consumer admits sees the same registries and toolkit. When different
people should see different tools, create one consumer per tool set, or let people
...
- [Consumers](/trustgate/concepts/consumers): slugs, routing, and the Connect tab
