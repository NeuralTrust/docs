...
[application SDK](/integrations/python-sdk) to evaluate content in your code.
...
| **[TrustGate](/trustgate/overview)** | Governs the traffic itself: which providers and models a [application](/trustgate/concepts/applications) may reach, request rate limits, token and dollar budgets, request-size guards, tool allow-listing, and which MCP tools exist at all | Which models · which tools · spend ceilings |
...
| Egress from the gateway deployment to `{TRUSTGUARD_BASE_URL}` | Configured through the deployment's `TRUSTGUARD_*` environment, not the policy form. Platform-managed on SaaS. |
...
<Step title="Create the TrustGate collector" titleSize="h2" id="1-create-the-trustgate-collector">
In TrustGuard, open **Agent Runtime → Collectors → Catalog → Gateway → TrustGate** and
...
<Step title="Assign a runtime policy" titleSize="h2" id="2-assign-a-runtime-policy">
On the collector's **Policies** tab, set a **default** policy, and per-consumer
overrides if one application needs different rules.
...
<Step title="Turn on the TrustGuard guardrail" titleSize="h2" id="3-turn-on-the-trustguard-guardrail">
...
for the gateway, or attached to one application.
...
<Step title="Choose Enforce or Observe" titleSize="h2" id="4-choose-enforce-or-observe">
...
not use a gateway, see the [application SDK](/integrations/python-sdk).
...
| --- | --- | --- | --- |
| `pre_request`, LLM route | `protocol: llm`, `direction: input` | Jailbreaks ([Prompt Guard](/trustguard/detectors/content-security#prompt-guard)); secrets and PII in the prompt ([DLP](/trustguard/detectors/data-loss-prevention)) | **Block** or **Redact** |
...
single application, so a gateway can inspect some applications and not others.
...
| A `tgk_…` key is rejected as an MCP credential | A collector key is not an MCP credential. MCP applications authenticate with OAuth or an `ag_…` key |
...
- [Per-Tool Rate Limiter](/trustgate/policies/per-tool-rate-limiter): limit MCP tool executions
