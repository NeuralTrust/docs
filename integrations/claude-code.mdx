...
| **[TrustGate](/trustgate/overview)** | Exposes the MCP registries and tools assigned to a consumer, and can proxy Claude Code's model traffic through an LLM consumer. MCP (Model Context Protocol) connects Claude Code to systems such as trackers, databases, and internal APIs. | Tool availability · consumer authentication · per-tool rate limits · model routing and budgets |
...
Claude Code can send its own model requests through an **LLM consumer** instead of
...
API; TrustGate serves it at `/{consumer_slug}/v1/messages` and routes it to whichever
registry the consumer binds — an Anthropic registry, or a cross-format route to an OpenAI
or Gemini registry.

1. Create an LLM consumer, bind the registries and models it may use, and issue an
   `ag_…` API key. Copy the LLM host from the **Connect** tab.
2. Point Claude Code at the consumer:

   ```bash
   export ANTHROPIC_BASE_URL="https://<llm-host>/<consumer-slug>"
...
TrustGate walks the consumer's registries in order and answers `404 model_not_supported`
...
is the consumer URL, then send a prompt and open the trace in the console. A `401` means
the key is wrong or revoked; a `404` means the slug or the path is wrong (the base URL must
end with the consumer slug, no `/v1`).
...
console; every caller the consumer admits sees the same tool set.
