...
| Requirement | Notes |
| --- | --- |
| Egress to `{TRUSTGUARD_URL}` | The console shows the URL for your workspace. The plugin calls it from the machine where Claude Code runs. |
| The **Claude Code** collector type | **Agent Runtime → Collectors → Catalog → IDE & coding agents → Claude Code**. Create its `tgk_…` key on **Auth** and assign the policy on **Policies**. The key is shown once. |
