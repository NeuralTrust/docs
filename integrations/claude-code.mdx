...
| The **Claude Code** collector type | **Runtime → Collectors → Catalog → IDE & coding agents → Claude Code**. Create its `tgk_…` key on **Auth** and assign the policy on **Policies**. The key is shown once. |
