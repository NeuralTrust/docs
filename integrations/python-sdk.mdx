...
| Requirement | Notes |
|---|---|
| A **Python SDK** [collector](/trustguard/concepts/collectors) and API key | Go to **Agent Runtime → Collectors → Catalog → Application → Python SDK**. Create the `tgk_…` key on the **Auth** tab, store it when shown, and load it from your environment or secret store. |
