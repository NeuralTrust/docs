...
[MCP application](/trustgate/mcp/overview) so tool access, identity, and rate limits
stay on the gateway.
...
```json
{
  "mcpServers": {
    "TrustGate": {
      "serverUrl": "https://<mcp-host>/<application>/mcp"
    }
  }
}
```

Use OAuth when the application requires it — Cascade starts the sign-in flow for
...
1. Confirm **TrustGate** is connected in the Cascade MCP panel and that its
   tools are listed.
2. Ask Cascade to use a tool from a toolkit bound to that application.
