...
**Report**, **Transform** or **Block**, so the verdict becomes a branch on the canvas
...
Policy can also apply across the turns of a conversation rather than to one message
at a time, which takes two options the node does not set for you — see
[Group the turns of a conversation](#7-group-the-turns-of-a-conversation).

## Before you start

| Requirement | Notes |
|---|---|
| An **Application** collector and its API key | The node calls the REST endpoint, so create **Agent Runtime → Collectors → Catalog → Application → REST API**. Create the `tgk_…` key on its **Auth** tab, where it is shown once, and assign a policy on the **Policies** tab. There is no n8n collector type — the key is what identifies the collector. |
| A policy bound to that collector | With Input **and** Output phase rules if you want both directions evaluated. |
| Egress from n8n to `{TRUSTGUARD_BASE_URL}` | The console shows the [base URL](/trustguard/api/evaluate#base-url) for your workspace. |
...
<Step title="Install the node" titleSize="h2" id="1-install-the-node">
...
<Step title="Create the credential" titleSize="h2" id="2-create-the-credential">
...
<Step title="Gate the input" titleSize="h2" id="3-gate-the-input">
...
| **Messages** | An OpenAI-style array as `payload.messages`, verbatim | A transcript you have assembled, including `tool_calls` and `tool` results. See [Group the turns of a conversation](#7-group-the-turns-of-a-conversation) |
...
<Step title="Scan the output" titleSize="h2" id="4-scan-the-output">
...
<Step title="Wire the four outputs" titleSize="h2" id="5-wire-the-four-outputs">
<img
  src="/images/integrations/n8n-verdict-routing.png"
  alt="n8n workflow showing TrustGuard's four outputs, with a Switch after Block separating Ask verdicts for human approval from blocked requests."
/>
...
| `ask` | **Block** | The node itself does not prompt for approval, so it routes `ask` to Block. `trustguard.status` remains `ask` for a separate approval step, as shown above. [Ask gates](/trustguard/concepts/policies#gates) match the input phase only. The item is not redacted. |
...
<Step title="Set the options" titleSize="h2" id="6-set-the-options">
...
Add **Session ID** so TrustGuard can evaluate a conversation instead of isolated messages. See
[Group the turns of a conversation](#7-group-the-turns-of-a-conversation) for what to point it at,
which is not always the prefilled expression. Add **Consumer ID** if the collector has
per-consumer [policy](/trustguard/concepts/policies) overrides, or every request resolves to the
default policy. In a chat workflow, use the trigger's authenticated user. **Protocol** should stay
`llm` for chat workflows.

<Warning>
Both **Session ID** and **Consumer ID** are dropped when their expression resolves to an empty
value, and neither raises an error. Unlike the **Text** parameter, which fails closed, an option
that resolves to nothing simply is not sent. Confirm both in **Activity** after wiring them.
</Warning>
</Step>

<Step title="Group the turns of a conversation" titleSize="h2" id="7-group-the-turns-of-a-conversation">
A multi-turn attack spreads its intent across several messages. Each one reads as harmless on its
own; the escalation, the reinforcement, or the reassembled instruction exists only across turns.
See [multi-turn attacks](/trusttest/create/threat-detection/prompt-injections/multi-turn/overview)
for the techniques involved — Crescendo, Echo Chamber, Multi-Turn Manipulation, and Payload
Splitting. Their common target is a check that inspects each message independently, which is what
the node does until you give it the conversation.

Two inputs give TrustGuard the conversation, and they are complementary rather than alternatives:

| What the node sends | What TrustGuard can then do |
|---|---|
| A stable `session_id` on **every** call of one conversation | Group the turns server-side. Findings for one conversation appear together in **Activity**, `session.id` becomes available as a [gate](/trustguard/concepts/policies#gates) condition, and the [stateful detectors](/trustguard/concepts/collectors#attribution) get the grouping they rely on. |
...
[synthesizes a `session_id` when the field is omitted](/trustguard/api/evaluate#request), so a
workflow that never sends one produces one session per request. Nothing errors and no finding goes
missing, so the gap is only visible as single-turn conversations in **Activity**.

### Point Session ID at a key that survives the workflow

The **Session ID** option is per node. Add it on **every** TrustGuard node in the workflow —
each one reads its own **Options** collection and builds its own request body — and point each at
an expression that actually resolves at that point on the canvas.

| Trigger | Expression to use |
|---|---|
| Chat Trigger, on a node wired directly to it | `{{ $json.sessionId }}`, the prefilled value. The trigger emits the chat client's own `sessionId`, which the client reuses for every message of that chat. |
| Chat Trigger, on any node after an AI Agent | `{{ $('When chat message received').first().json.sessionId }}`, naming your trigger. |
| Webhook | Your own field: `{{ $json.body.session_id }}`, or a header such as `{{ $json.headers['x-session-id'] }}`. A webhook item has no `sessionId`. |

<Warning>
The prefilled `{{ $json.sessionId }}` is correct on **Evaluate Input** and wrong on
**Evaluate Output** in a chat workflow. The TrustGuard node preserves the incoming item, but an
**AI Agent** node replaces it with its own result, so `sessionId` is gone by the time the output
node runs and the expression resolves to nothing. Reference the trigger by name instead, or put a
**Set** node with **Include Other Input Fields** enabled after the agent.
</Warning>

Align the value with your memory node's session key so the conversation TrustGuard groups and the
conversation the agent remembers are the same one. Every n8n chat-memory node exposes
**Session ID** as either **Connected Chat Trigger Node** or **Define below**; whichever expression
it resolves, use the same one in the TrustGuard option.

`{{ $execution.id }}` and `{{ $workflow.id }}` are not conversation keys. A chat turn is one
execution, so the first changes every turn, and the second is shared by every conversation in the
workflow.

### Give one call the history

Set **Input Mode** to **Messages** and point the **Messages** parameter at an assembled
OpenAI-style array. The node sends it verbatim as `payload.messages`, so one evaluation call
carries the conversation, with `tool_calls` and `tool` results intact.

Build the array in a **Code** node from whatever your workflow already stores. To read an
attached memory node's history onto the `main` path, use a **Chat Memory Manager** node in
**Get Many Messages** mode, then map its output in a Code node: its entries are keyed by sender
(`human`, `ai`, `system`) rather than by `role`, and the node fails closed on an array whose
items have no `role`.

The whole transcript is re-sent and re-scanned on every turn, and the node evaluates one item per
call. In Messages mode the node writes its own normalized array back to `json.messages` on every
branch, not only on `transform`, so a downstream node that appends the next turn is reading the
node's copy rather than the one it assembled.
</Step>

<Step title="Verify" titleSize="h2" id="8-verify">
...
| --- | --- | --- | --- |
| **Evaluate Input** | `protocol: llm`, `direction: input`, `payload.messages`; role `user` in Text mode | Jailbreaks ([Prompt Guard](/trustguard/detectors/content-security#prompt-guard)); secrets and PII in the prompt ([DLP](/trustguard/detectors/data-loss-prevention)) | **Block** output |
...
`consumer_id`, `session_id`, and `attributes.model.provider` are sent only when set — meaning the
option was added *and* its expression resolved to a non-empty value. Each node builds its own
body, so an option set on one TrustGuard node does not apply to another. See
[Group the turns of a conversation](#7-group-the-turns-of-a-conversation).
...
| Input turns group but output turns do not | The output node kept the prefilled `{{ $json.sessionId }}`, and the AI Agent replaced the item, so the expression resolves to nothing. Reference the trigger by name |
| A **Session ID** or **Consumer ID** option is set but never arrives | The expression resolved to an empty value. An option that resolves to nothing is dropped silently; only the **Text** parameter fails closed |
| Turns group in **Activity** but the agent does not remember them | The memory node's session key is a different value than the one sent as `session_id`. Use one expression for both |
| Each turn is a separate session even though a key is sent | The expression is `{{ $execution.id }}`, which is a new value per chat turn |
