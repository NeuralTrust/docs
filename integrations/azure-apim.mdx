...
| Requirement | Notes |
|---|---|
| An Azure APIM collector, API key, and assigned policy | Go to **Agent Runtime → Collectors → Catalog → Gateway → Azure APIM**. Create the collector, create the `tgk_…` key on its **Auth** tab, and assign a policy with the required Input and Output rules on the **Policies** tab. The key is shown once and identifies the collector. |
| Egress from the APIM instance to `{TRUSTGUARD_BASE_URL}` | The console shows the [base URL](/trustguard/api/evaluate#base-url) for your workspace. |
...
<Step title="Create the collector and API key" titleSize="h2" id="1-create-the-collector-and-api-key">
1. Open **Agent Runtime → Collectors → Catalog → Gateway → Azure APIM** and add the
   collector.
...
<Step title="Screen the prompt on inbound" titleSize="h2" id="2-screen-the-prompt-on-inbound">
...
<inbound>
  <send-request mode="new" response-variable-name="guard" timeout="10">
    <set-url>{TRUSTGUARD_BASE_URL}/v1/evaluate</set-url>
...
<Step title="Screen the completion on outbound" titleSize="h2" id="3-screen-the-completion-on-outbound">
...
| --- | --- | --- | --- |
| `<inbound>` | `protocol: llm`, `direction: input`, `{ "input": "<raw request body>" }` | Jailbreaks ([Prompt Guard](/trustguard/detectors/content-security#prompt-guard)); secrets and PII in the prompt ([DLP](/trustguard/detectors/data-loss-prevention)) | **Block** returns `403`; `transform` rewrites the body |
...
| --- | --- | --- |
| `<set-url>` | `{TRUSTGUARD_BASE_URL}/v1/evaluate` | The console shows the URL for your workspace |
...
| No events in **Activity** | No policy assigned to the collector, or `{TRUSTGUARD_BASE_URL}` was never substituted |
