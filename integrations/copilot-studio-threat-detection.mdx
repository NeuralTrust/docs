...
immediately before a tool runs. This page says what that hook covers, then
...
| **Knowledge** and **Connected agents** | ◐ | Covered whenever the platform runs them as a tool execution, which is how it orchestrates them. *Not yet confirmed against live traffic — confirm on your agent in Activity.* |
...
inside Microsoft's platform.** A prompt is evaluated only when it lands in the
arguments of a tool about to run. The verdict is one boolean, so use **Block**
when content must not reach the tool — **Transform** does not redact on this
path.

Coverage follows the platform's orchestration, not a list we maintain. To
...
value the other two ask for. Visit Entra twice, in that order: step 2 creates
the application so you have its ID; step 4 adds the credential, which encodes
an endpoint the collector only produces once it knows the application ID.

Configuration is **per environment**, with no tenant-level setting and no API.
Every new environment repeats the collector and Power Platform steps.
...
| An agent using **generative orchestration** | These integrations apply to Copilot Studio agents that use generative orchestration. |
...
<Step title="Copy the threat detection endpoint" titleSize="h2" id="1-copy-the-threat-detection-endpoint">
...
<Step title="Register the client application in Microsoft Entra ID" titleSize="h2" id="2-register-the-client-application-in-microsoft-entra-id">
...
Two applications are involved, and you create only one. This one is the
*client*: Copilot Studio authenticates as it, it holds the federated credential
from [step 4](#4-add-the-federated-identity-credential), and its Application
(client) ID is what you paste into Power Platform.

The *audience* application claims your endpoint's origin as its **Application
ID URI**, so Entra knows who the token is for. That one is **NeuralTrust's**.
Entra only accepts an Application ID URI on a domain verified in the tenant
that owns the application, and the endpoint is on our domain. Its client ID is
never entered anywhere. If it is missing from your tenant,
[step 5](#5-turn-on-threat-detection-in-power-platform-admin-center) fails with
`AADSTS500011` — contact us rather than registering it yourself.
</Step>

<Step title="Store the tenant and application IDs on the collector" titleSize="h2" id="3-store-the-tenant-and-application-ids-on-the-collector">
...
<Step title="Add the federated identity credential" titleSize="h2" id="4-add-the-federated-identity-credential">
...
Leave **Expose an API**, App roles, and API permissions empty. The token is
issued against a federated credential, machine to machine; scopes are never
evaluated.

<Warning>
**Copy the subject identifier from the Connection tab. Never build it by
hand.** It encodes your tenant *and* the exact endpoint URL. The tenant GUID is
re-ordered into .NET byte order and base64url-encoded, so a subject assembled
from the tenant ID looks reasonable, **Entra accepts it without complaint**,
and it never matches an incoming token.

If the endpoint changes, both the Application ID URI (origin) and the
credential subject (full URL) go stale. Add the new URI — an application
accepts several. The subject cannot be edited after creation: delete the
credential and add one with the subject the Connection tab now shows.
</Warning>

These five identifiers look alike; mixing them up is the usual setup error:
...
If a Microsoft portal asks for a value, it comes from Microsoft or from the
collector's Connection tab — never from the NeuralTrust console address bar.
</Step>

<Step title="Turn on threat detection in Power Platform admin center" titleSize="h2" id="5-turn-on-threat-detection-in-power-platform-admin-center">
...
step 1 and the Application (client) ID from step 2. The provider list is empty
until you type in the filter — search for your provider by name.

Power Platform admin center also asks what should happen when TrustGuard cannot
answer in time.

<Warning>
**Copilot Studio waits 1000 ms for a verdict and treats a slower answer as no
answer.** That limit is Microsoft's, it is measured from their side, and it
covers the network in both directions as well as the evaluation itself.
...
Choose deliberately, because this setting decides what happens to a delivery
Microsoft never got an answer for — not to one we allowed. With **block the
query**, anything that misses the window stops the tool, and the maker sees
*"the security check could not be completed"* rather than a block from your
policy. With **let the agent respond**, the tool runs unscreened. Neither is
safer in the abstract: one fails toward availability, the other toward caution.
</Warning>
...
<Step title="Assign a policy" titleSize="h2" id="6-assign-a-policy">
On the collector's **Policies** tab, assign a default policy. A collector with
no matching policy is unguarded: TrustGuard allows every tool and inspects
nothing. Start in **Observe** so findings are recorded without stopping any
tool, check **Agent Runtime → Activity**, then switch to **Enforce**.
...
Read the blocked call in **Agent Runtime → Activity** before moving on. In a
real run of this test the proposed call was impeccable — a correct recipient,
an ordinary subject, a well-written body, and the model had not obeyed the
injection at all — and it was blocked anyway. The evidence was entirely in what
...
The person using the agent does not see your `reason`. Copilot Studio shows its
own message — *"This action was blocked by threat detection tools. Please
contact your administrator if you believe this is an error."* — and tells the
agent to relay that text verbatim. Your `reason` and `diagnostics` are for
logs and Activity.

In the maker's trace the blocked call carries `errorCode: "pre-tool-blocked"`
with *"This action was blocked before the tool could run."* That pair
distinguishes a block from every other reason a tool call can fail.

### Failure behavior

| Situation | What happens |
|---|---|
| Verdict not ready within **1000 ms** | Copilot Studio stops waiting and treats the delivery as unanswered. The environment's error behavior decides whether the tool runs. |
...
- [Governed tool access](/integrations/copilot-studio-governed-tool-access): coverage and setup for the other path
