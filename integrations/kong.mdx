...
| Requirement | Notes |
|---|---|
| A Kong collector, API key, and assigned policy | Go to **Agent Runtime → Collectors → Catalog → Gateway → Kong**. Create the collector, create a key on its **Auth** tab, and assign a policy with the required Input and Output rules on the **Policies** tab. The key is shown once and identifies the collector. |
| Egress from Kong to `{TRUSTGUARD_BASE_URL}` | The console shows the [base URL](/trustguard/api/evaluate#base-url) for your workspace. |
...
<Step title="Create a collector API key" titleSize="h2" id="1-create-a-collector-api-key">
...
<Step title="Configure AI Proxy on the route" titleSize="h2" id="2-configure-ai-proxy-on-the-route">
...
<Step title="Add the guardrail plugin" titleSize="h2" id="3-add-the-guardrail-plugin">
...
plugins:
  - name: ai-custom-guardrail
    config:
      ...
      request:
        url: "{TRUSTGUARD_BASE_URL}/v1/evaluate"
        headers:
...
<Step title="Map the verdict to a block" titleSize="h2" id="4-map-the-verdict-to-a-block">
...
<Step title="Configure response evaluation" titleSize="h2" id="5-configure-response-evaluation">
...
<Step title="Verify" titleSize="h2" id="6-verify">
...
evaluated direction adds one round trip to `{TRUSTGUARD_BASE_URL}`.

### What is evaluated

| Kong pass | TrustGuard | What you can stop | Enforcement |
| --- | --- | --- | --- |
| Request pass | `protocol: llm`, `direction: input`; `payload.input` contains the request text flattened by `text_source` | Jailbreaks ([Prompt Guard](/trustguard/detectors/content-security#prompt-guard)); secrets and PII in prompts ([DLP](/trustguard/detectors/data-loss-prevention)) | **Block** prevents the provider call |
...
| `request.url` | `{TRUSTGUARD_BASE_URL}/v1/evaluate`, including the full endpoint path. |
...
| No events in **Activity** | No policy assigned to the collector, a wrong `request.url`, or no egress from Kong to `{TRUSTGUARD_BASE_URL}` |
