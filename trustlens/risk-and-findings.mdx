...
Applies to AI SaaS usage observed across the organization.
...
TrustLens identifies *what* needs protection. [Agent Runtime by TrustGate](/trustgate/overview) enforces *how* it is protected at runtime.

A common pattern: a TrustLens **FAIL** on `guardrails_configured` becomes the trigger to put that agent behind a Gateway with a [prompt-security policy](/trustgate/overview) attached. Once the Gateway is in front, the control will pass on the next sync with a reference back to the runtime policy.
