...
<Note>
Endpoint Discovery is **not** an enforcement surface — it does not block, modify, or proxy any traffic. For runtime enforcement on endpoint devices, see the [Endpoint enforcement surface](/trustgate/platform/enforcement/endpoint) under TrustGate Runtime.
</Note>
...
| MCP runtime activity | Discovery reports declared MCP servers from config files. To observe what tools an MCP server is actually invoked with, use the [MCP tool security](/trustgate/platform/mcp-tool-security) features of TrustGate Runtime. |
