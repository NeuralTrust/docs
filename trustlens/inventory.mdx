...
**Populated by:** The Runtime **browser extension** deployed to managed browsers. The extension reports the AI SaaS domains users visit back to TrustLens — no prompt or response content is captured, only the tenant-level visit. See [Runtime enforcement surfaces → Browser](/trustgate/platform/enforcement/browser).
