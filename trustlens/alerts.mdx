...
3. **Contain (if user-facing).** If the agent is public or org-wide and a *Critical* control is failing (no guardrails, no auth, computer-use enabled), put the agent behind [TrustGate Runtime](/trustgate/overview) with a hardening policy until the underlying findings are resolved.
...
4. **Configure enforcement.** Once approved, register the SaaS in the policy.
...
| 2. Adversary exploits the weakness | [Telemetry alert](/platform/alerts) | Jailbreak-driven tool-call surge on the same agent. |
...
TrustLens alerts trigger on **posture** changes — what a resource looks like at rest. [Telemetry alerts](/platform/alerts) trigger on **traffic** anomalies — what a resource does in flight, from TrustGuard and TrustGate telemetry.

A typical workflow: a TrustLens *Critical* alert on `function_tools_scope` for a production agent prompts the team to attach a [tool-permission policy](/trustgate/overview) at the runtime layer. The runtime alert then watches for unexpected tool-call patterns once the policy is in place.
