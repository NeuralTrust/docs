...
| `direction` | enum | — | `input` (default) or `output`. Selects which [policy](/trustguard/concepts/policies) detector phase runs. [TrustGate](/trustguard/integrations/trustgate) sets this for you; other collectors must send it on every call (usually twice per turn). |
