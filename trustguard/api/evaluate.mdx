...
`POST /v1/evaluate` is the standard runtime endpoint for TrustGuard collectors.
A [collector](/trustguard/concepts/collectors) calls it to evaluate one request
against its assigned [policy](/trustguard/concepts/policies). See
[How it works](/trustguard/how-it-works) for policy actions and collector coverage.
Claude Enterprise uses the specialized endpoint documented in its
...
`{TRUSTGUARD_URL}` throughout these guides:
