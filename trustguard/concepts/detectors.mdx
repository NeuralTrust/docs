...
- `anomaly_detector` — retention window and behavioral scoring weights
  (hidden from the catalog picker; see the [catalog](/trustguard/detectors/overview)).

See each detector's full settings on its
[category page](/trustguard/detectors/overview).

The Agent Runtime catalog picker exposes the seven detectors returned by
`GET /v1/plugins`. `code_sanitation`, `multiturn_guard`, and `anomaly_detector`
are registered but omitted from that list.
