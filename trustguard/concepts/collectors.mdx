---
title: "Collectors"
description: "A collector is a traffic tap — a gateway, SDK, browser extension, or WAF — that calls /v1/guard. It authenticates with an API key and routes each request to a policy."
---
...
gateway, the gateway calls `/v1/guard` for you and enforces the verdict
...
- **`consumer_id`** — who made the request (user id, email, or device
  fingerprint). Used for per‑consumer routing and by `anomaly_detector`.
