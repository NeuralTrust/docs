...
[TrustGuard overview](/trustguard/overview#building-blocks). Browse
...
**Two gateways still need a piece you build on current releases.**
[Portkey](/integrations/portkey) calls a Bring Your Own Guardrails webhook adapter you
host. [LiteLLM](/integrations/litellm) has a native `neuraltrust` guardrail contributed
upstream, but until a release carries it the proxy loads a custom guardrail class you
place beside its `config.yaml`. Both work today; both are more setup than the other
groups.
