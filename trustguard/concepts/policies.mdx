...
Rules run in order for the phase. A **`block`** finding **cuts the detection chain** (later detector rules do not run). Otherwise completed rules reduce to the **most restrictive** action.
...
gates, no detectors) and returns `status: "allow"`.
