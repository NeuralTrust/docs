...
| **Ask** | `ask` | Requests confirmation where the host provides an approval flow. This action applies to input only. |
| **Transform** | `transform` | The host forwards `transformed_payload` instead of the original content. Available for [DLP](/trustguard/detectors/data-loss-prevention) (`data_loss_prevention`) outcomes only. |
...
| [n8n](/integrations/n8n#5-wire-the-four-outputs) | **Routed to Block.** The `trustguard.status` value remains `ask`, so a workflow can branch to an explicit approval step. Workflow connections determine whether execution continues. Since version 0.2.0, the node no longer fails on this status. |
| [LangChain](/integrations/langchain) | **Blocked** because the middleware has no `ask` case and fails closed on unknown verdicts. |
| [Claude Enterprise](/integrations/claude-enterprise) | **Allowed** because this integration returns only allow or deny and has no interactive prompt. |
...
| [LiteLLM](/integrations/litellm) | **Blocked** on the native `neuraltrust` guardrail, which returns HTTP `400` naming `verdict: ask` because the proxy has no approval flow. **Allowed** and logged on the custom guardrail file. |
| Other gateways | **Allowed**, and recorded. There is no user to prompt and the verdict is not `block`, so the request passes through. |
...
| [Claude Enterprise](/integrations/claude-enterprise) | Agent platforms | Anthropic servers; no local installation | Every model request from Claude chat, Claude Code and Cowork | Anthropic organization administrator (`organization:manage`) |
| [Claude Code](/integrations/claude-code) | AI coding agents | Developer machines | Prompts, tool calls and tool results | IT through MDM; developers need no NeuralTrust account |
...
**Streaming responses:** TrustGate and the LiteLLM custom guardrail file inspect a
streamed response after it has been delivered to the client, so findings on
streamed output are monitor-only. The native LiteLLM guardrail samples the stream
as it accumulates and a block ends it, but chunks already sent stay with the
client. The other gateway integrations cover only non-streaming responses.

**LiteLLM redaction:** Not available on any released LiteLLM version. The native
`neuraltrust` guardrail contributed in BerriAI/litellm#37165 applies `transform`
on input and on non-streaming output; these tables will change once a release
carries it. The custom guardrail file does not redact.
...
bound tools, and the
[Claude Enterprise](/integrations/claude-enterprise) transcript.
...
**LiteLLM:** Either guardrail runs before and after model calls and applies
verdicts at the request level. The native `neuraltrust` guardrail applies
`transformed_payload` on non-streaming completions, writes back transformed
tool calls, lands `ask` as a block, and fails closed on a transform it cannot
apply safely; it is not in a released LiteLLM version yet. The custom guardrail
file does not apply `transformed_payload`. On a streamed response the custom
guardrail file evaluates the output after delivery for monitoring only, while
the native guardrail samples the accumulating text and a block ends the stream;
neither can recall chunks that have already been sent.
...
| If you need | Use |
| --- | --- |
| Masking sensitive data in flight | [TrustGate](/integrations/trustgate) for non-streaming LLM responses and MCP traffic; [LiteLLM](/integrations/litellm) for non-streaming completions once a release carries the native guardrail |
| Coverage of streaming completions | [TrustGate](/integrations/trustgate) or [LiteLLM](/integrations/litellm) for monitoring after delivery, and the native LiteLLM guardrail to end a stream mid-way; place preventive rules on the request path |
...
| Organization-wide Claude coverage without local installation | [Claude Enterprise](/integrations/claude-enterprise) |
