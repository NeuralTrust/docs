---
title: "How it works"
description: "The /v1/guard pipeline: resolve the collector from its API key, resolve the policy, run gates, run the detector rules, and reduce everything to a single status the caller enforces."
---

Every inspection goes through one runtime endpoint, `POST /v1/guard`. This page
...
| `findings[]` | One entry per gate or detector that fired. See the [Guard API](/trustguard/api/guard) for the full shape. |
