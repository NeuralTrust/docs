...
TrustGuard has five building blocks. You compose them once, then send traffic.

| Concept | What it is |
|---|---|
| **Plugin** | A built‑in detection capability from the [catalog](/trustguard/detectors/overview) (e.g. `prompt_guard`, `data_loss_prevention`). Detection‑only. |
| **[Detector](/trustguard/concepts/detectors)** | A reusable, named instance of a plugin plus its `settings` (thresholds, entity lists, …). Still detection‑only — it decides *what* it finds, not what to do. |
...
```text
        POST /v1/guard  (Authorization: Bearer <collector api key>)
                              │
 ┌──────────┐  resolves  ┌────▼─────┐  routes to  ┌──────────────────────────────┐
