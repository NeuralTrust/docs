...
and calls [`/v1/guard`](/trustguard/api/guard).
...
- **DLP at the source** — catch PII/secrets before they leave the browser, using a
  [`data_loss_prevention`](/trustguard/detectors/data-loss-prevention) detector in
  `redact` or `block`.
...
- Use a dedicated collector (and detector chain) for browser traffic so its policies and
