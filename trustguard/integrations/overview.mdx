...
description: "Every way to send traffic to TrustGuard — gateway, application SDK, browser, and edge/WAF collectors — all using the same /v1/guard call."
---

A [collector](/trustguard/concepts/collectors) is wherever you call
[`/v1/guard`](/trustguard/api/guard). When you create or open a collector in the console,
...
two calls per interaction. See the full contract in the [Guard API](/trustguard/api/guard).

## Two rules for every integration

1. **Create the API key on the collector's Auth tab** and pass it as
...
take the base URL and call `/v1/guard` for you:
...
key and detector chain, and behavioral signals correlate across them by `consumer_id`.
