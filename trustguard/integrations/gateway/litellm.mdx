...
**Use it when** you run a LiteLLM proxy and can add a Python file and restart it.
**Not when** you do not control the proxy deployment, or you need masking — the
guardrail raises on `block` and does not consume `transformed_payload`.

**Limits.** `default_on: true` matters: without it the guardrail only runs for
requests that name it, so callers can opt out by omission. Both `pre_call` and
`post_call` are needed for full direction coverage.
