...
`multiturn_guard` is registered but **hidden** from the catalog picker — see below.
...
stable `session_id` on the [`/v1/evaluate`](/trustguard/api/evaluate) request.
...
<Note>
`multiturn_guard` is **hidden** from Agent Runtime → Detectors → Catalog
(`GET /v1/plugins`). It depends on Redis and the session‑analysis worker.
Existing detectors keep working; contact NeuralTrust to enable it for your team.
</Note>
