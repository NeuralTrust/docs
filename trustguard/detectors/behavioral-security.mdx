...
- Pair with [rate limiting at TrustGate](/trustgate/getting-started/overview) — rate
  limits cap volume, `anomaly_detector` catches *patterns* that stay under the cap.
- Start in `observe` and tune the weights/threshold against real traffic before moving
  to `block`, since behavioral signals are probabilistic.
