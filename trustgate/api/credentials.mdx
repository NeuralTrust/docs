...
plane only. The data planes — Proxy (`8081`) and MCP (`8082`) — take an application's own
credentials rather than these tokens, and are documented under
[Endpoints](/trustgate/endpoints/chat) and [MCP](/trustgate/mcp/overview).
...
interactive identity. Machine credentials cannot call them even when the gateway binding
is correct.
