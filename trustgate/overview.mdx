...
- **Cost & abuse control** — request rate limiting, token/dollar **LLM budgets**,
  request-size guards, and embedding-based **semantic caching** to cut spend on repeated
  prompts.
- **Tool & prompt governance** — inject operator-authored tools, rate-limit tool calls,
  inject and version system prompts; restrict which models a consumer may reach via
  [`model_policies`](/trustgate/routing/model-resolution).
