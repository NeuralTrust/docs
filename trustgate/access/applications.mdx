...
| A person's own sign-in (NeuralTrust login or your identity provider) | That person | Someone present, authenticating as themselves. |

One application does all three, one call at a time — a nightly job and a
customer-facing assistant can be the same application without a setting deciding
which it is allowed to be. See [acting for end users](/trustgate/mcp/end-users)
for the middle row.
