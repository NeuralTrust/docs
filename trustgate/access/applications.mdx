...
An application is served on the planes it uses: **Models** for inference,
**Tools** for MCP, or both. Each plane it uses gives it an endpoint to call.

An application with neither is not yet an application — it has nowhere to serve,
and the console will not let you finish it until you add a provider or a server.
