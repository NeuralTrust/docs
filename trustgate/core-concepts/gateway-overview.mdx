...
A gateway in AI Gateway is a logical grouping of configurations that defines how API traffic should be managed, secured, and routed. Each gateway is identified by api keys and can be configured with its own set of plugins, services, and routing rules.
...
API Keys are used to authenticate and authorize access to the proxy provided by the AI Gateway. Each client must include a valid API key in their requests to route through the gateway.
...
**Traffic management settings** control how requests flow through each service, enabling fine-grained control over service behavior and performance. These settings encompass rate limiting configurations, timeout values, and retry policies. They allow administrators to optimize service performance, protect backend resources, and ensure reliable operation under varying load conditions.
...
- [Upstream & Services](upstream-services-overview)
- [Rules](rules)
- [Plugins](plugin)
