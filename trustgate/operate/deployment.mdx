---
title: "Deployment"
description: "Run TrustGate's planes as independently scalable processes via Docker or Kubernetes, backed by Postgres and Redis."
---

TrustGate is a single static binary plus two datastores (Postgres, Redis). Because the
planes are selected by an argument, you scale **Admin**, **Proxy**, and **MCP**
independently. Telemetry is exported with **OpenTelemetry** to a collector — not a
message bus.
...
Compose files: `docker-compose.yaml` (infra — Postgres, Redis), plus
...
plane needs. Wire `OTEL_EXPORTER_OTLP_*` to your OpenTelemetry Collector (or use the
NeuralTrust Hybrid chart, which configures metadata export for you).
...
Point your load balancer at `/readyz` so a plane only receives traffic once Postgres and
Redis are reachable.
