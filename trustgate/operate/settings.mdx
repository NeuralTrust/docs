---
title: "Settings"
description: "What belongs to the gateway itself: its URLs, how it was deployed, who may call the Admin API, which identity providers and certificate authorities it trusts, and its transport floor."
---
...
your private plane on Hybrid — and the **MCP Store** URL people point clients at.
...
providers whose tokens it accepts, and the certificate authorities for mTLS. This
is where "let services holding an Okta token call the gateway" and "let this CA's
client certificates authenticate" are configured — see [Auth](/trustgate/concepts/auth).

## Networking

The transport floor: **minimum TLS version** — older clients are rejected —
**HSTS** for browser connections, with max-age, subdomains and preload, and
whether **mTLS** is required on incoming traffic.
