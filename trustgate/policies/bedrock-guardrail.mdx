...
AWS decides, per entity, whether a match is reported as `BLOCKED` or
`ANONYMIZED`. TrustGate's **PII action** decides only what the gateway does when
AWS reports `ANONYMIZED` — and its default, *Block request*, **overrides AWS's
anonymise verdict into a hard block**. A guardrail configured to mask email
addresses, dropped into a policy left at its defaults, refuses the request
instead.

Real masking needs both: the entity set to *Anonymize* in the AWS console **and**
**PII action** set to *Anonymize* here. An entity AWS reports as `BLOCKED` is
...
`ANONYMIZED` verdict from AWS, and it does nothing at all unless the entity is
set to anonymise on the AWS side.

**Block message** — leave your expectations low here.

<Warning>
**The block message is stored and never used.** The field is saved with the
policy, and the `403` body carries the matched policy and the matched name —
never this text. Do not rely on it to tell your callers anything.
</Warning>
...
The request never reaches the model. The caller gets `403` and a body naming the
policy that fired and, where AWS provides one, the name it matched:
...
```json
{
  "error": {
    ...
    "policy": "topic_policy",
    "name": "DangerousTopics"
  }
}
```

`policy` is one of `topic_policy`, `content_policy`, `word_policy`,
`sensitive_information` or `contextual_grounding`.
</Step>
<Step title="Masking applies only if nothing blocked">
With **PII action** set to *Anonymize*, and AWS reporting `ANONYMIZED`, the
masked text AWS returned replaces the original message and the request continues
to the model. The model never sees the original.
</Step>
...
|---|---|---|
| A topic, content, word or grounding policy reports `BLOCKED` | `403`, refused | Recorded, request continues |
| Sensitive information reports `BLOCKED` | `403`, refused | Recorded, request continues |
| Sensitive information reports `ANONYMIZED`, **PII action** *Block request* | **`403`, refused** | Recorded, original text sent |
| Sensitive information reports `ANONYMIZED`, **PII action** *Anonymize* | Rewritten in place | Recorded, **original text sent** |
...
Expect the decision **`block`**, a `403` whose body names the policy that fired,
and **`Provider 0 ms`** in the timing breakdown. That zero is the evidence that
matters: the request never reached the model.
</Step>
</Steps>
...
| `403` with `"policy": "sensitive_information"` although **PII action** is *Anonymize* | AWS returned no masked text, or the request body could not be re-encoded | Expected, and deliberate: the original is never forwarded when masking fails |
...
| Your **Block message** never reaches the caller | It is stored and never read | Expected; the body carries the matched policy and name |
