...
<Warning>
**Only the last user message is sent on the request leg.** Not the system
prompt. Not the earlier turns. Not tool definitions, tool calls or tool results.
Not attachments. The gateway walks back through the messages, takes the most
recent one whose role is `user`, and sends that single string — and if there is
no user message with text in it, the request is forwarded without calling Model
Armor at all.
...
<Step title="Create the inspection template — the first half of masking">
This step and the next one are a pair, and masking needs both. Skip **both** if
you only want to block sensitive data and never rewrite it; skipping only this
one leaves the next step with nothing to pair with, and the Model Armor template
in step 4 asks for the two paths together.
...
The form offers two ways, and the first is the one to prefer.
...
| **[Service account key](#method-b--service-account-key)** | An encrypted copy of the private key | Deleting the key in Google Cloud and pasting a new one | Impersonation is unavailable to you. |
...
replace it rather than read it. Google Cloud hands you the key file once, when
you create it, so keep your own copy where you keep your other secrets. Prefer
impersonation where it is available, rotate on your normal schedule, and delete
the key in Google Cloud once impersonation becomes possible.

<Warning>
**The credential field for the authentication you picked cannot be left empty.**
The form shows one field at a time — the email, or the key — and refuses to save
until it has a value.
</Warning>

<Note>
**A deployment can also call Model Armor as the gateway's own identity**, with
no credential on the policy at all. That is arranged at the deployment, not on
this screen, and the form will not produce it for you. It only fits a
self-hosted, single-tenant gateway — on a shared one every policy would call as
the same identity, which would need a grant in every tenant's project — and that
identity needs `roles/modelarmor.user` on your project. Ask whoever manages your
gateway.
</Note>
...
| **Authentication** | *Impersonate a service account* (Method A) or *Explicit credentials* (Method B). The form draws one credential field at a time. |
| **Service account email** | Method A only. Required. |
| **Service account key (JSON)** | Method B only. Required. |

### Block on

Five toggles — **Sensitive data**, **Responsible AI**, **Prompt injection &
jailbreak**, **Malicious URIs**, **CSAM**. A new policy starts with all five on,
and at least one must stay on or the policy will not save.

<Warning>
**These toggles decide what is *evaluated*, not only what is blocked.** A filter
you leave off is never read out of Model Armor's answer: it cannot refuse a
request, and nothing about it reaches the event either, so there is no "detect
it but do not block on it" for a single filter. Turning off **Sensitive data**
also turns off masking: the *Sensitive data action* below has no effect unless
that toggle is on.
</Warning>
...
response reads the model's answer and can mask sensitive data in it the same
way. Each leg is its own call to Model Armor, so screening both roughly doubles
what the policy costs per turn. Start with the request, measure, and add the
response leg once you know what it costs you.
...
never reaches the model. The caller gets `403`. The refusal wording is fixed —
there is nothing to configure — and it names the filter that fired: sensitive
data, Responsible AI, prompt injection and jailbreak, malicious URIs, or CSAM.
</Step>
...
| A filter selected in **Block on** did not run | **`502`, refused** | Recorded, request continues |
...
Model Armor being unreachable, or a credential that cannot be resolved, records
on every request.
...
<Note>
**A filter that did not run counts as a failure, not as a clean result.** Model
Armor can answer successfully and still report that one of the filters you
selected in **Block on** never executed. Silence from a filter that did not run
looks exactly like silence from a filter that found nothing, so rather than read
one as the other the policy refuses with a `502` — a correct configuration and a
reachable Model Armor, and the request is still refused. Deselect the filter if
you do not need it, or check it is still configured on the Model Armor
template.
</Note>
...
- **Only the last user message is inspected on the request leg.** No system
  ...
  results.
- **Masking that fails is a block.** If Model Armor returns no de-identified
  text, or the request format cannot be re-encoded with it, the request is
  refused rather than forwarded. The original is never sent.
- **A filter that did not run fails the request closed.** See the note above.
- **Each leg is a separate call.** A policy screening both the request and the
  response calls Model Armor twice per turn.
...
going on. In **Enforce**, a `502` here, recorded as **`failed_closed`**, means
the gateway could not get an answer out of Model Armor at all: the location, the
...
With *Anonymize* and the policy in **Enforce**, expect the decision
**`anonymized`**. In **Observe** the same prompt records **`reported`** and the
original text still reaches the model, so switch to **Enforce** before reading
this step.
...
Expect the decision **`block`**. To prove the request never reached the model,
open **Activity**, open the request, and read its **Policies** tab: the
**Aggregated Latency** breakdown there has a **Provider** row, and that row is
empty. The Playground shows a single latency figure and does not break it down.
</Step>
</Steps>
...
| `502` on every request in Enforce, recorded as **`failed_closed`** | Model Armor unreachable, or the credential cannot be resolved | Check the location, then confirm the impersonation grant is on the service account and not on the project |
| `502` although the configuration is correct and Model Armor is reachable | A filter selected in **Block on** came back without having run | Deselect it if you do not need it, or check it is still configured on the Model Armor template |
| The policy will not save | The credential field for the selected authentication is empty, **Project**, **Location** or **Template ID** is empty, or no filter is selected in **Block on** | The console names the field; fill it in, or select at least one filter |
...
- [AWS Bedrock Guardrail](/trustgate/policies/bedrock-guardrail) — the other provider guardrail that can mask in place
