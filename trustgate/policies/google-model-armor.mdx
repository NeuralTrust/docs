...
never reaches the model. The caller gets `403`, and the response names the filter
that fired — sensitive data, Responsible AI, prompt injection and jailbreak,
malicious URIs, or CSAM.
</Step>
...
Expect the decision **`block`**, and **no time at the provider** in the timing
breakdown. That the provider leg is empty is the evidence that matters: the
