...
<Warning>
**This policy adds a synchronous call to every request.** Measured with the
gateway and Model Armor in the same region: roughly **450–800 ms** per leg.
Enable it on the request leg first and measure before adding the response leg,
which doubles the added latency per turn.
</Warning>
...
the policy. Pick a region close to your gateway — you pay that network latency on
every request.
...
Expect the decision **`block`** and **`Provider 0 ms`** in the latency
breakdown. That zero is the evidence that matters: the request never reached the
model.
</Step>
</Steps>

Decisions are recorded under [Activity](/trustgate/observability/metrics),
including in Observe mode where nothing is blocked.
...
| The first request after a credential change is slow | A fresh access token is being minted | Expected once per credential, then cached |
...
- [Activity](/trustgate/observability/metrics) — where decisions are recorded
