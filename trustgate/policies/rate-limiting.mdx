...
Both respect [policy](/trustgate/policies/overview) scope (**Gateway-wide** or **Targeted** applications) and support **Enforce** / **Observe** (and throttle where the plugin allows).
...
3. Set **Scope** (mode + gateway-wide / targeted applications).
4. Fill the form (details below).
5. Save. Verify with **Playground** or
   **Analytics → Policy** / **Cost**.
...
| **Targeted applications** | Per-tenant or per-app quotas. |
| **Group by header** | Fairness inside one application (per end-user) without a policy per user. |
...
- [Smart routing](/trustgate/llm/routing#smart-routing) — route by Simple / Medium / Hard labels; pair with LLM Budget for spend ceilings
