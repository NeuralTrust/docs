...
[policy](/trustgate/policies/overview) scope — per consumer, or gateway-wide — and accept a
group-by header to sub-partition the counter (e.g. per end-user or tenant).
...
4. Fill limits, windows, and on-exceed behavior in the form.
5. Save. Verify with [Playground](/trustgate/console/playground) or
   [Analytics → Policy](/trustgate/console/analytics).

For a **per-request** price ceiling (not a running budget), see
[Cost cap](/trustgate/policies/cost-cap).
