...
description: "Run your own TrustGuard detectors at the gateway, over LLM and MCP alike — the only guardrail that sees tool calls, and the only one whose Observe mode can still refuse a request."
---
...
It is the only guardrail in the catalogue that covers **LLM and MCP** — the
Model Context Protocol your agents call tools over. Every other one sees chat
text only. It is also the only one that sends the whole
conversation — system prompt, every message with its role, assistant
`tool_calls`, the tool definitions the client declared, and file and image
attachments — rather than a single flattened string.
...
| Leg | LLM | MCP |
|---|---|---|
| Request | System prompt, every message with its role, assistant `tool_calls`, `tool_call_id`, the declared tool definitions, and image or file attachments | The tool name and every string value in the call arguments |
| Response | Assistant content, reasoning text when present, and `tool_calls`, plus the request's tool definitions for context | Text blocks, the text of embedded resources, and every string leaf of `structuredContent` |
...
The caller gets `403` and a body naming what fired:

```json
{
  "status": "block",
  "message": "Request blocked by security policy: prompt_injection.",
...
The detector or gate name is appended to the message, and carried as its own
field, when the finding has one. **The findings themselves are not in the body** —
no matched text, no confidence scores. Those are on the event and in Activity.
...
- a TrustGuard **rate limit** — `429`, with `Retry-After`, `X-RateLimit-Limit`,
  `X-RateLimit-Remaining` and `X-RateLimit-Reason` forwarded to the caller;
...
Expect the decision **`block`**, a `403`, and **`Provider 0 ms`** in the timing
...
This one is not a prompt, so do it from a client already connected to an MCP
server in this policy's scope — whatever your agents use. Call a tool with an
argument that should trip the same detector.
...
| `429` with `Retry-After`, including in Observe | TrustGuard rate limit for the plan | Expected — this path never consults the mode. Wait out `Retry-After` |
...
| `502 trustguard_unauthorized` | The collector was deleted, or belongs to another gateway, or the credential was refused | Re-select the collector and confirm it is bound to this gateway |
