...
## Part 1 — Set up Azure AI Content Safety (work in progress)

<Note>
**This part is still being written.** What is below has been confirmed in a real
Azure portal and is safe to rely on. What is missing is the last stretch of the
walkthrough — the region list, the pricing tiers on offer, and the exact page
the key and endpoint are copied from. Those are being captured now.
</Note>

Everything in this part happens in **your own** Azure subscription. TrustGate
never creates or changes anything there.

### Before anything else: do you have a subscription?

This is the step that stops people, and it is not about permissions.

<Warning>
**A Microsoft tenant is not an Azure subscription, and having the first does not
give you the second.** A company using Microsoft 365 has a Microsoft Entra
directory — users, groups, sign-in — and very often no Azure subscription at
all. Without one there is nowhere for a resource to exist, so nothing can be
created.

The symptom is not an error. The Content safety page shows a cheerful *Welcome
to Azure!* panel offering a free trial, which reads like a product tour rather
than the blocker it is.

Being an administrator of the directory does not help either: directory roles
and Azure roles are separate systems. You can administer the tenant and still
have no access to Azure.
</Warning>

Check it before you start: search for **Subscriptions** in the portal's search
bar. An empty list means someone has to create a subscription — a free trial, a
pay-as-you-go, or access to an existing one — and grant you a role on it. That
is a billing decision, not a technical one, so it is worth starting early.

### Where the service lives

Content Safety is not a standalone service in the portal any more. It sits under
**Microsoft Foundry**, reachable from **More services → Content safety** in the
left-hand navigation, alongside the other Azure AI services.

### What you will not find in Azure

Worth saying plainly, because anyone arriving from the Bedrock or Model Armor
pages will go looking for it:
...
That makes this the lightest provider setup of the three. You create a resource
and copy two values out of it.
</Note>
...
| The **Analyze Text** endpoint URL, **including its `api-version` query string** | Built from the resource endpoint on the portal's **Keys and Endpoint** page — not that endpoint on its own. See the warning at the top of this page. |
