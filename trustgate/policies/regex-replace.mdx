...
It is the only guardrail in the catalogue that **can never block**. Every other
one decides; this one edits. Reach for it when you know the exact shape of what
must not pass — an internal hostname, a ticket reference, a key prefix — and you
want that removed without a detector's opinion in the loop.
...
| Field | What it is |
|---|---|
| **Pattern** | An RE2 regular expression. Required. |
...
The event records `no_match`. A rule whose replacement is identical to what it
matched counts as no change.
</Step>
...
| No rule matches | Forwarded untouched, event `no_match` | Forwarded untouched, event `no_match` |
...
- **A rewrite re-encodes the whole body, and fields the gateway does not model
  are lost.** When a rule fires, the request is rebuilt from the gateway's own
  ...
  handful of sampling settings. Everything else at the top level is dropped —
  `seed`, `n`, `user`, the penalties, `logit_bias`, `stream_options`, `store`
  and `metadata` on an OpenAI-shaped body, `thinking` on an Anthropic one,
  `safetySettings` and `cachedContent` on a Gemini one; the response leg loses
  `created`, `system_fingerprint` and `service_tier`. A body no rule matched is
  forwarded byte for byte, so only the requests this policy actually rewrites
  are affected.
...
Expect a normal reply and no rewrite — the event records `no_match`. If this
fails, the problem is the application or the model, not the rules. Fix it before
going on.
</Step>
