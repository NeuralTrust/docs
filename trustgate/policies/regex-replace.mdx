...
{/* TODO(verify): the request body is re-encoded from the gateway's canonical
    representation rather than patched in place. Whether provider-specific
    fields the canonical form does not model survive that round trip is not
    established from the plugin alone — confirm against the provider adapters
    before documenting it either way. */}
