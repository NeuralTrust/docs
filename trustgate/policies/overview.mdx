...
| [`cost_cap`](/trustgate/policies/cost-cap) ("LLM Cost Cap") | Per-request guard on a model's list price per 1k tokens. |
...
| [`model_allowlist`](/trustgate/policies/prompt-model-controls) | Restrict which models a consumer/gateway may call. |
| [`tool_allowlist`](/trustgate/policies/tool-governance) | Allow/deny the tools in a request `tools[]` array. |
...
| Policy (`slug`) | Purpose |
|---|---|
| [`tool_call_validation`](/trustgate/policies/tool-governance) | Validate / redact the tool calls a model returns. |
| [`tool_definition_transformation`](/trustgate/policies/tool-governance) | Patch tool schemas, override descriptions, and inject tools. |
