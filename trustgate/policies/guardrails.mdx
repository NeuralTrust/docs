---
title: "Guardrails"
description: "Inspect or rewrite prompts and responses inline at the gateway with TrustGuard, OpenAI Moderation, Azure Content Safety, AWS Bedrock, Google Model Armor, or Regex Replace policies."
---
...
2. Pick **TrustGuard**, **OpenAI Moderation**, **Azure Content Safety**,
   **Bedrock Guardrail**, **Google Model Armor**, or **Regex Replace**.
...
| [`google_model_armor`](/trustgate/policies/google-model-armor) | Google Cloud Model Armor | `pre_request` · `pre_response` |
...
The **Google Model Armor** policy screens prompts and responses with
[Google Cloud Model Armor](https://cloud.google.com/security-command-center/docs/model-armor-overview).
One call covers sensitive data, Responsible AI categories, prompt injection and
jailbreak, malicious URLs, and CSAM. It blocks, or replaces detected sensitive
data in place with the de-identified text Model Armor returns. Streaming
responses pass through untouched.

Setting it up means building a template in Google Cloud and granting the gateway
access to it, so it has a guide of its own:
...
- **`google_model_armor`:** reuse a Model Armor template defined in Google Cloud,
  including in-place masking of sensitive data.
