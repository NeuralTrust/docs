...
counters. The event is exported as an OpenTelemetry log record to the collector configured
for the deployment (NeuralTrust SaaS, or your own on Hybrid).
...
routing **attempts**, and the **policy chain**. The field-level contract used by alerts and
analytics is the [Event schema](/platform/event-schema).

Identity on the event is two separate things: the **principal** that authenticated, and
the **end user** a client application declared it was serving. See
[End-user attribution](/trustgate/observability/end-user-attribution) for how a chat
front-end behind one API key attributes each of its people.
