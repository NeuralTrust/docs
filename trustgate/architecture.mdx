...
1. A client calls the proxy with a consumer API key (or OAuth2/OIDC token, or client cert).
...
Gateway discovery prefers the **`X-AG-Gateway-Slug` header**, then falls back to the
request `Host` / subdomain (`{slug}.<GATEWAY_BASE_DOMAIN>`). SaaS typically relies on
subdomain; Private/Hybrid often sends the header.

`GATEWAY_DISCOVERY_MODE` (if set) can bias toward header-first or subdomain-first behavior —
prefer documenting the header → Host fallback above when troubleshooting.
