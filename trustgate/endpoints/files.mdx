---
title: "Files"
description: "Call /{application}/v1/files to upload, list, retrieve, delete, and download content. TrustGate routes only to registries that expose a Files store."
---
...
```bash
curl "https://<gateway-host>/<application-slug>/v1/files?purpose=assistants" \
  -H "X-AG-API-Key: <your-api-key>"

curl "https://<gateway-host>/<application-slug>/v1/files/<file-id>" \
  -H "X-AG-API-Key: <your-api-key>"

curl "https://<gateway-host>/<application-slug>/v1/files/<file-id>/content" \
  -H "X-AG-API-Key: <your-api-key>"

curl -X DELETE "https://<gateway-host>/<application-slug>/v1/files/<file-id>" \
  -H "X-AG-API-Key: <your-api-key>"
```
...
OpenAI SDKs use the same application base URL: `client.files.create()`, `list()`,
...
1. The application's registries are filtered to those that advertise files.
...
Attach at least one files-capable registry to the application. A mixed chat + files pool is
fine: chat still uses every chat-capable member; files skip the rest.

## Related

- [Providers](/trustgate/concepts/providers) — which adapters expose files
- [Registries](/trustgate/concepts/registries)
- [Applications](/trustgate/access/applications) — Connect tab
- [Models](/trustgate/endpoints/models) — discovery does not list file ids
