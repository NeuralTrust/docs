---
title: "Images"
description: "Call POST /{application}/v1/images/generations (JSON) plus multipart /edits and /variations. TrustGate routes only to registries that can generate images."
---
...
```bash
curl -X POST "https://<gateway-host>/<application-slug>/v1/images/edits" \
  -H "X-AG-API-Key: <your-api-key>" \
  -F model=dall-e-2 \
  -F prompt="make it blue" \
  -F image=@logo.png

curl -X POST "https://<gateway-host>/<application-slug>/v1/images/variations" \
  -H "X-AG-API-Key: <your-api-key>" \
  -F model=dall-e-2 \
  -F image=@logo.png
```
...
OpenAI SDKs use the same application base URL: `client.images.generate()`, `client.images.edit()`,
...
1. The application's registries are filtered to those that advertise images.
...
Attach at least one images-capable registry to the application. A mixed chat + images pool is
fine: chat still uses every chat-capable member; images skip the rest.

## Related

- [Providers](/trustgate/concepts/providers) — which adapters generate images
