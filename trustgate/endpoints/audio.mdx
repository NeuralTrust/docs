---
title: "Audio"
description: "Call POST /{application}/v1/audio/speech (TTS, raw bytes) and /v1/audio/transcriptions (STT, multipart). TrustGate routes only to registries that can speak or transcribe."
---
...
```bash
curl -X POST "https://<gateway-host>/<application-slug>/v1/audio/speech" \
...
```bash
curl -X POST "https://<gateway-host>/<application-slug>/v1/audio/transcriptions" \
  -H "X-AG-API-Key: <your-api-key>" \
  -F model=whisper-1 \
  -F file=@speech.mp3
```
...
OpenAI SDKs use the same application base URL: `client.audio.speech.create()` and
...
1. The application's registries are filtered to those that advertise the matching capability
...
Attach at least one audio-capable registry to the application. A mixed chat + audio pool is
fine: chat still uses every chat-capable member; speech and transcriptions skip the rest.

## Related

- [Providers](/trustgate/concepts/providers) — which adapters do speech and transcription
