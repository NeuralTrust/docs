---
title: "Embeddings"
description: "Call POST /{application}/v1/embeddings with an OpenAI-shaped body. TrustGate routes only to registries that can embed."
---
...
1. The application's registries are filtered to those that advertise embeddings.
...
Attach at least one embeddings-capable registry to the application. A mixed chat + embeddings
pool is fine: chat still uses every chat-capable member; embeddings skip the rest.

## Related

- [Providers](/trustgate/concepts/providers) — which adapters can embed
