...
Client-side TLS secures TrustGate's outbound connections to upstream services. This ensures encrypted communication between TrustGate and external services, such as APIs or backend servers. Configure client-side TLS during gateway creation via the TrustGate API or within configuration files.
