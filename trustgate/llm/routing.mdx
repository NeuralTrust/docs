---
title: "Routing"
description: "How an application picks a registry and a model for each request: one target, a tier chosen by how hard the prompt is, an ordered fallback chain, or a load-balanced pool — and what the client has to send for each."
---
...
There are four answers, and the console offers them as one **Strategy** choice.
...
| **Smart routing** | A tier, by how hard the prompt is. | Two or more model strengths serve the same application and cost should follow difficulty. |
...
| **Load balancing** | A member of a pool, by algorithm. | Traffic should spread — evenly, by weight, or by load. |
...
The console lists it beside the load-balancing algorithms, but it is a different
kind of decision: not *which member takes the next request* but *which model
this request deserves*.
