...
The key travels as `X-AG-API-Key`, `x-api-key`, `x-goog-api-key` or
`Authorization: Bearer` — a bearer that starts with `ag_` is recognised as an
application key — so the OpenAI, Anthropic and Google SDKs work unchanged with
their own `api_key` parameter.
...
[TrustGate SDK](/sdks/trustgate/overview), the key alone configures the model
client, base URL included. That is the one value no client can work out on its
own, because the LLM plane lives on a different host from the MCP one.
...
<CodeGroup>
  ...
  const llm = await new TrustGate({ apiKey: "ag_…" }).llm()
  const openai = new OpenAI({ baseURL: llm.baseUrl, apiKey: llm.apiKey })
  ```
</CodeGroup>

Nothing is wrapped: streaming and every provider option keep working as the
provider's own client implements them. [Models](/sdks/trustgate/models) covers
Anthropic's client, the other fields `llm()` returns, and naming the user.
