...
Anthropic's client takes `llm.anthropicBaseUrl` instead (`anthropic_base_url` in
Python). It appends `/v1/messages` itself, so the base it needs stops at the
application; `baseUrl` already ends in the `/v1` OpenAI's client expects.
