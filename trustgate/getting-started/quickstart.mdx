...
<p className="nt-lead">
  Create a gateway in the console, connect a provider, send a request, and open
  the first trace. Choose SaaS unless prompts must stay in your network.
</p>

- **SaaS** — NeuralTrust hosts the data plane. Nothing to install.
- **Private (Hybrid)** — you run the data plane; the SaaS console still manages it.

Choose **Private** for data residency, private networking, deterministic capacity,
or a production SLA. See [Installation](/trustgate/getting-started/install).
...
response. Continue to **Playground**, **Application**, or **Security** from the next-step
cards.
</Step>
</Steps>
...
1. Open **Applications**.
2. Select **Application**; the **New Application** panel opens. You can also open an existing
   application.
3. For a new application, select **API Key** under **Authentication** and copy the generated
   key when it is shown—it appears only once. For an existing application, manage
   credentials under **Auth**.
4. Open **Connect** and use a snippet under **Connection details**.

The provider credential remains in TrustGate. Keep the application API key secret and use
...
```bash
curl -X POST "https://<gateway-host>/<application-slug>/v1/chat/completions" \
...
```bash
curl -X POST "<configured-llm-url>/<application-slug>/v1/chat/completions" \
...
Embeddings use the same host, application slug, and key:

```bash
curl -X POST "https://<gateway-host>/<application-slug>/v1/embeddings" \
  -H "X-AG-API-Key: <your-api-key>" \
  -H "Content-Type: application/json" \
  -d '{
...
Images use the same host, application slug, and key:

```bash
curl -X POST "https://<gateway-host>/<application-slug>/v1/images/generations" \
  -H "X-AG-API-Key: <your-api-key>" \
  -H "Content-Type: application/json" \
  -d '{
    "model": "dall-e-3",
...
Files use the same host, application slug, and key:

```bash
curl -X POST "https://<gateway-host>/<application-slug>/v1/files" \
  -H "X-AG-API-Key: <your-api-key>" \
...
From Getting started **What's next**, open **Playground**, **Application**, or **Security**
(policies) — or continue below.

<CardGroup cols={2}>
  <Card title="Endpoints" icon="plug" href="/trustgate/endpoints/chat">
    Every route a client can call through a application.
  </Card>
  <Card title="Event schema" icon="activity" href="/platform/event-schema">
    The per-request event behind Activity, Analytics and alerts.
  </Card>
  <Card title="Applications" icon="users" href="/trustgate/access/applications">
