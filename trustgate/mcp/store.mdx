---
title: "Store & Portal"
description: "The self-service side of MCP: people browse a catalogue, install servers for themselves, connect their own accounts, and ask for what they cannot have — while admins keep the shelf."
---

Applications are for things your team built. The **Store** is for people.

Every gateway serves a Store at one fixed MCP URL. A person points their MCP
client at it, signs in with their NeuralTrust login, and gets three tools: search
the catalogue, install a server, uninstall it. Everything they install appears on
that same URL. The **Portal** is the same catalogue as a web page, for people who
would rather click than ask an agent.

Neither governs applications and applications do not govern them. An
application's surface is what an admin bound; a person's Store surface is what
they installed, bounded by their [Access](/trustgate/access/overview) level.
...
|---|---|---|
| **Installed** | Tools are on the Store URL now. | Nothing. |
...
A gateway's **default access** decides what the Store offers everyone who has no
level of their own:
...
| **None** | The Store is closed: search answers nothing, installs are refused. |
...
Store asks which one to install or uninstall, and each holds its own set of
per-user accounts.

## Where it is

The Store URL is under the gateway's settings, next to the LLM and MCP gateway
URLs. Employees without console access reach the Portal from the platform; admins
