...
Create an MCP [application](/trustgate/access/applications) with an API key and set its
...
source yet on a new application, it only shows it once set. Bind the MCP registries the
application needs; servers with `forwarded` upstream auth are the ones each user will connect.

| Request header | Value |
|---|---|
| `X-AG-API-Key` | The application's API key (also accepted as `x-api-key` or `Authorization: Bearer ag_…`). |
| `X-NeuralTrust-End-User` | Your id for the person: an opaque string, at most 256 characters, no control characters. **Required** on every MCP request to this application. |

The request runs as principal `app:<consumer_id>:<end_user>`. Vault credentials, the
consent flow, and `tools/list_changed` pushes are all per end user. Two applications never
...
set is the application's, and who may use your application is your application's decision.

## Connections API

Two endpoints on the MCP plane, next to the application, authenticated with the application's
API key. Requests without the header, or against a application that does not identify its end
users, are refused.
...
the application that forwards a credential. The ticket is redeemable for **15 minutes**. Every
...
| HTTP | `error` | Meaning |
|---|---|---|
| `400` | `invalid_request` | Bad body, bad `end_user`, or a `provider` that is not a connectable server of this application. |
| `401` | `unauthenticated` | Wrong API key, or the slug is not an MCP application of this gateway. |
| `409` | `end_users_not_identified` | The application's `identity.source` is not `app`. |
| `429` | — | Connect-attempt rate limit, per application and per source. Honour `Retry-After`. |
...
## Attribution without acting for users

If all you want is to *see* which of your users a request belonged to — in Activity, in
analytics, in cost reports — you do not need any of the above. That is
[End-user attribution](/trustgate/observability/end-user-attribution): send
`X-TG-User-Id` / `-Email` / `-Name` / `-Role` (or let Open WebUI send its own headers) and
TrustGate records the person with no application configuration at all.

The difference is what the value is allowed to do. Attribution is telemetry: asserted by
the caller, never verified, never reaching a policy or a credential. The end user on this
page is identity: it becomes the request's principal, and it decides whose upstream
accounts the call may use — which is why it requires a application that opts into it.

## Related

- [Applications](/trustgate/access/applications): `identity`, API keys, and registry bindings
