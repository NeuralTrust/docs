...
the same application still contribute theirs.
...
<Steps>
  ...
  <Step title="Connect">
    Save, then bind the server to an [MCP application](/trustgate/mcp/overview) like any
    other. Use a **toolkit** to choose which operations that application may actually use.
  </Step>
</Steps>
...
| *This document exposes N tools* | The document defines more than 80 tools. Use a toolkit on the application to limit the exposed set. |
...
sign in to TrustGate, which is set on the application.
...
- [MCP Gateway](/trustgate/mcp/overview): applications, toolkits, and upstream authentication
