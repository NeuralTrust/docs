...
3. Enter the **OpenAPI spec URL** (`http`/`https`, OpenAPI 3.x). Optionally set **API
   base URL** to override `servers[0]`.
...
The Admin plane publishes its own OpenAPI document (`docs/openapi.json` in the TrustGate
repo). You can register that spec as an OpenAPI MCP source to drive Admin from an agent
— useful for dogfooding and for verifying compilation against a large real document.

1. Serve or publish the spec at an `https` URL the data plane can fetch (or point
   `spec_url` at Admin if you expose `/openapi.json` / swagger).
2. Set **API base URL** (`mcp_target.url`) to the Admin origin (for example
   `https://admin.example:8080`). The generated document uses `servers: [{ "url": "/" }]`,
   so without an override the base URL is resolved relative to the spec URL.
