...
1. **That the tools you need are there.** The tool set belongs to an admin and
   can be narrowed without warning. `requires` turns that into a refusal at
   startup instead of a failure mid-conversation.
2. **That the servers behind it have an account.** A batch has nobody to open a
   ...
   account every caller rides on, or you, by naming the person a call is for —
   see [acting for your users](#acting-for-your-users).
...
`GET {mcp-host}/whoami`, with the key in any of the headers above, is what lets a
client be configured with one secret instead of a secret and two URLs:

```json
{
  "gateway": "acme",
  "key": { "name": "batch", "expires_at": "2026-12-01T00:00:00Z" },
  "consumers": [
    {
      "slug": "triage", "type": "MCP", "active": true,
      "url": "https://<mcp-host>/triage/mcp",
      "upstreams": [
        { "server": "Linear", "provider": "linear", "account": "user",
...
`expires_at` is absent when the key never expires. `upstreams` covers the servers
behind an MCP application that read a stored account, answered for the
application itself: `blocked` is `administrator` for a shared account nobody has
connected, `end_user` for a server that keeps an account per caller, and absent
when the server is ready. An absent `upstreams` is not an empty one — a gateway
that could not read the accounts omits it too, so read `blocked`, never a length.

Everything after that is ordinary MCP over streamable HTTP at the `url` above,
...
when it expires, and for each server whether an account is connected and who
connects it. It is the first thing to run when something is off.
