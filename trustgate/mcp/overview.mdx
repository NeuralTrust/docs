...
the consumer's **toolkit** (`mcp.toolkit`), which lists grants that scope a registry to
...
identical for every caller the consumer admits: the [Access](/trustgate/mcp/access) page
governs the Store only and never narrows a consumer. To give two audiences different tools,
create two [consumers](/trustgate/concepts/consumers).
...
Every MCP consumer with at least one MCP server bound also lists a few tools TrustGate
...
| `trustgate_store_search` · `trustgate_store_install` · `trustgate_store_uninstall` | Only on the [Store](/trustgate/mcp/store) consumer. |

A consumer whose toolkit is a deny-all (a toolkit with no `tool` entry) gets no gateway
tools either.
...
An MCP consumer called with an API key or a client certificate runs as the application
...
| `POST /v1/gateways/{gateway_id}/consumers/{id}/upstream-accounts/link?registry_id=` | Mints a connect ticket: `{ticket, consumer_path, connect_path, providers[], expires_at}`. Omit `registry_id` to cover every forwarding server. `404` for a registry not bound to the consumer; `409` for one that carries its own credential or when the consumer acts for users. |
