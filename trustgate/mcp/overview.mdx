...
Register a server as a [registry](/trustgate/concepts/registries) of `type: MCP` with an
`mcp_target`:
...
Browse pre-seeded enterprise servers via `GET /v1/mcp-servers-catalog`, validate a
