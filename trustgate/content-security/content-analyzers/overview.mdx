...
<CardGroup cols={2}>
  <Card title="Document Analyzer" icon="file-pdf" href="/trustgate/content-security/content-analyzers/doc-analyzer">
    Scan uploaded files (PDF, DOCX, images) for PII, jailbreaks, and sensitive content
  </Card>
...
| Setting | Description |
|---------|-------------|
| `mode` | `enforce` to block threats, `observe` to log only |
| `pii.entities` | List of PII types to detect |
| `firewall.enabled` | Enable jailbreak detection |
| `firewall.threshold` | Jailbreak score threshold (0.0-1.0) |
| `firewall.credentials` | NeuralTrust API credentials for jailbreak detection |
