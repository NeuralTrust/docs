...
| **Short name** | `gpt-4o-mini` | TrustGate walks the consumer's registries in configured order and uses the first one that serves the model. A name no registry serves is rejected with **404** `model_not_supported`. |
...
Model restriction is configured on the **consumer**, not as a Policies catalog plugin.
