---
title: "Smart routing"
description: "Route each request by prompt complexity across Simple, Medium, and Hard tiers — pick cheaper models for easy prompts and stronger models for hard ones, configured on the application Routing tab."
---
...
Each label can be used **at most once** per application. You need **at least two** tiers
...
1. The client calls the application (typically with `"model": "auto"` — see
...
and the application's pool falls back to **round-robin**.
...
| **Tier registries must belong to the application** | Checked on every save, even while load balancing is disabled, so a tier can never keep a registry that no longer resolves. |
...
The application **Connect** tab and **Playground** already use
**Auto** for this strategy. Do not hard-code a single model id unless you intentionally
want to bypass complexity selection (and the model is allowed on the application).
...
- [Model resolution](/trustgate/routing/model-resolution) — `auto`, short names, and model
  filters on the application.
- [Applications](/trustgate/access/applications) — Routing tab overview.
