---
title: "Okta"
description: "End-to-end manual for wiring Okta to TrustGate: a custom authorization server and scopes, validating Okta bearer tokens on an application, and OAuth2 login for MCP."
---

This manual sets up Okta as the identity provider for two TrustGate patterns in TrustGate (the trust anchor under **Settings → Machine identity**, then the
application's **Authentication**):

1. **Validate bearer tokens** — an LLM or MCP application whose callers already hold an Okta
...
2. **OAuth2 · MCP login** — an MCP application gated by a custom scope. Interactive agents
...
<Steps>
  ...
  <Step title="Attach it to the application">
    ...
    - **Allowed client IDs**: the Okta **Client ID** of every application that may call
      this application. TrustGate matches the token's `azp` claim, else `client_id`. Okta
...
1. The agent discovers TrustGate as the authorization server
   ...
   `/{application-slug}/mcp` — and `/.well-known/oauth-authorization-server`).
...
3. TrustGate redirects the browser to Okta's authorize endpoint using the **Web app**
   ...
   `redirect_uri={mcp_base_url}/oauth/callback` (host root — not under the application
   path).
...
5. If the virtual MCP has upstream providers (Notion, Linear, …), TrustGate may show a
   **Connect your accounts** page at `/{application-slug}/mcp/connect` *after* Okta
...
standalone M2M curl tests — never paste that client into the MCP application's OAuth2 auth.
</Warning>
...
<Steps>
  ...
  <Step title="Connect from Cursor">
    Open the application → **Connect** tab. Copy the Cursor snippet (URL only — no API key):
...
| Authorize URL still has the old `client_id` | Auth not updated under **Machine identity**, or the MCP client is pointing at a different gateway / application. |
...
| `403 forbidden` on a bearer call | Token verified but its `azp` / `client_id` is not in the application's **Allowed client IDs** — or the token only carries Okta's `cid` and the custom `client_id` claim from [§3](#3-validate-okta-bearer-tokens-llm-or-mcp) is missing. |
