---
title: "Entra ID"
description: "End-to-end manual for wiring Microsoft Entra ID (Azure AD) to TrustGate: an app registration with an exposed API scope, validating Entra bearer tokens on a consumer, and OAuth2 login for MCP."
---
...
1. **Validate bearer tokens** — an LLM or MCP consumer whose callers already hold an Entra
...
2. **OAuth2 · MCP login** — an MCP consumer gated by an exposed API scope. Interactive
...
<Steps>
  ...
  <Step title="Attach it to the consumer">
    ...
    - **Allowed client IDs**: the **Application (client) ID** of every calling
      ...
      is dedicated to this one consumer.
...
| `403 forbidden` on a bearer call | Token verified but its `azp` is not in the consumer's **Allowed client IDs**. |
