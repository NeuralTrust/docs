...
| **`role_based`** | The consumer's [roles](/trustgate/concepts/roles); roles are selected from IDP token claims. | IDP JWT. |
...
Next: authenticate consumers with [auth credentials](/trustgate/concepts/auth).
