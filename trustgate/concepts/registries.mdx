...
|---|---|---|
| **LLM** | A model provider endpoint (OpenAI, Anthropic, Bedrock, …). | Chat / Responses / Messages / [embeddings](/trustgate/routing/embeddings). |
...
Set this on a connected LLM registry so [Analytics](/trustgate/console/analytics) **Cost**,
...
Overrides win. If a model has no override, the discount is applied to the models.dev
catalog price. Otherwise the catalog price is used as-is. A model with no catalog price
is unpriced: the event has no `cost`, and a dollar budget accrues `0` for that request.
