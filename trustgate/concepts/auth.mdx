...
| **mTLS** | Client certificate (or a trusted `X-Forwarded-Client-Cert` header) | Static consumers only — [mTLS](/trustgate/concepts/mtls) |
...
**mTLS** is created with the Admin API (not the **New Auth** form) and attaches to Static
consumers on both planes. See [mTLS](/trustgate/concepts/mtls).
