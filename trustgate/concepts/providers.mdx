...
| Moonshot (Kimi) | `moonshot` | Yes | — | — |
| OpenRouter | `openrouter` | Yes | — | — |
| xAI | `xai` | Yes | — | — |
| Custom / OpenAI-compatible | `openai_compatible` | Yes | Yes | — |

**Moonshot (Kimi)** speaks the OpenAI Chat Completions wire format on the
international endpoint (`api.moonshot.ai`). Its catalog entries come from the
models.dev key `moonshotai` (for example `kimi-k3`). For the China endpoint
(`api.moonshot.cn`), use **Custom / OpenAI-compatible** with that `base_url`.
...
| Provider | Auth |
|---|---|
| OpenAI, Anthropic, Gemini, Mistral, Moonshot, Groq, Cohere, DeepSeek, xAI, Cerebras, OpenRouter | API key |
