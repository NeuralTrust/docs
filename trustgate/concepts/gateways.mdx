---
title: "Gateways"
description: "A gateway is the top-level tenant in TrustGate — created in the console, it owns the registries, applications, auth, and policies beneath it."
---

<p className="nt-lead">
  A gateway is the top-level isolation boundary. Registries, applications, auth,
  and policies belong to exactly one gateway. Run one per team,
  environment, or product and switch between them in the console.
</p>
...
See [Installation](/neuraltrust/deployment/overview) and the
...
Connection details for apps also appear on each application's **Connect** tab.

## Delete a gateway

Deleting a gateway in the console removes everything it owns (registries, applications, keys,
trust anchors, policies). Config changes apply immediately to the data plane.
